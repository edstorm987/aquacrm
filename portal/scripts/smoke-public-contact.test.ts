...
// These two components live in a sibling checkout, not this repository. A
// missing sibling must skip only its own contract instead of preventing every
// in-repository assertion in this file from loading.
...
const hasAquaOasis = existsSync(join(AQUAOASIS_ROOT, "website/components/ChatBot.tsx"))
  && existsSync(join(AQUAOASIS_ROOT, "website/components/SupportForm.tsx"));
const aquaChatbot = hasAquaOasis
  ? readFileSync(join(AQUAOASIS_ROOT, "website/components/ChatBot.tsx"), "utf8")
  : "";
const aquaSupport = hasAquaOasis
  ? readFileSync(join(AQUAOASIS_ROOT, "website/components/SupportForm.tsx"), "utf8")
  : "";
...
test("aquaoasis chatbot and support forms declare their channel", {
  skip: hasAquaOasis ? false : "aquaoasis-web sibling checkout not present",
}, () => {
