...
describe("End-customer portal sub-routes (R019)", () => {
  ...
  it("/portal/customer/account/page.tsx is a built-in customer account page", () => {
    ...
    assert.ok(src.includes("resolveUserAuthContext(user, { clientId: session.clientId })"));
    assert.ok(src.includes("...(authContext.client ? { clientId: authContext.client.id } : {})"));
    assert.ok(src.includes('href={`/login/forgot?${new URLSearchParams({'));
