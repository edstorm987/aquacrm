...
interface AuthorityRecord {
  ...
  captureId: string;
  captureEmail: string;
  grant: Omit<PendingCapturePromotionAuthorityGrant, "subjectKey"> & { subjectKey?: string };
}
...
interface World {
  ...
  inspect: {
    ...
    erasureCalls: string[];
    erasureFailures: { remaining: number };
  };
}
...
function buildWorld(): World {
  ...
  const erasureFailures = { remaining: 0 };
  let pendingSeq = 1;
  ...
  const promotions: PendingCapturePromotionPort = {
    async promote(input) {
      ...
      return {
        ...
        personOwned: true,
        prospectOwned: true,
        pipelineCardOwned: true,
      };
    },
  };
  const promotionErasure: PendingCaptureErasurePort = {
    async erase(input) {
      erasureCalls.push(input.captureId);
      if (erasureFailures.remaining > 0) {
        erasureFailures.remaining -= 1;
        throw new Error("injected_crm_erasure_failure");
      }
      return { recordsErased: 4, sharedIdentityPreserved: false };
    },
  };
  const promotionAuthority: PendingCapturePromotionAuthorityPort = {
    async verify(input) {
      ...
      return {
        ...record.grant,
        subjectKey: record.grant.subjectKey ?? `actor:${record.grant.actorUserId}`,
      };
    },
  };
  return {
    storage, activity, events: eventBus, leadUsers, promotionAuthority, promotions, promotionErasure,
    inspect: {
      activityLog,
      events,
...
function container(world: World) {
  return containerWithDeps({
    ...
    promotionErasure: world.promotionErasure,
  });
}
...
describe("@aqua/plugin-public-funnel smoke", () => {
  ...
  test("24. authority revoked after the first check is revalidated inside the promotion lane", async () => {
    setClock(() => T0);
    const w = buildWorld();
    const c = container(w);
    const captured = await c.funnel.captureHcCompletion({
      email: "revoked-before-lock@example.com",
      completionId: "revoked_before_lock_01",
      slot: { slot: 3 },
    });
    let checks = 0;
    w.promotionAuthority.verify = async input => {
      checks += 1;
      if (checks > 1) return null;
      return {
        kind: "authenticated",
        operationId: "revoked-command-001",
        actorUserId: "agency_owner_001",
        subjectKey: "user:agency_owner_001",
      };
    };
    await assert.rejects(
      () => c.funnel.promotePendingCapture({
        ...
        credential: {
          kind: "authenticated",
          sessionToken: "signed-then-revoked",
          operationId: "revoked-command-001",
        },
      }),
      (error: unknown) => error instanceof FunnelInputError
        && error.message === "promotion_authority_refused",
    );
    assert.equal(checks, 2, "authority was not resolved again inside the durable lane");
    assert.equal(w.inspect.promotionCalls.length, 0);
    assert.deepEqual(await w.storage.list("promotion-claims/"), []);
    assert.ok((await c.funnel.list())[0]?.pendingLeadId, "revoked authority changed the pending row");
    resetClock();
  });

  test("25. one authenticated subject/operation cannot promote two captures", async () => {
    setClock(() => T0);
    const w = buildWorld();
    const c = container(w);
    const first = await c.funnel.captureHcCompletion({
      email: "operation-capture-a@example.com",
      completionId: "operation_capture_a_01",
      slot: { slot: 2 },
    });
    const second = await c.funnel.captureHcCompletion({
      email: "operation-capture-b@example.com",
      completionId: "operation_capture_b_01",
      slot: { slot: 4 },
    });
    w.promotionAuthority.verify = async input => input.credential.kind === "authenticated"
      ? {
          kind: "authenticated",
          operationId: "shared-command-001",
          actorUserId: "agency_owner_001",
          subjectKey: "user:agency_owner_001",
        }
      : null;
    const credential = {
      ...
      sessionToken: "same-signed-session",
      operationId: "shared-command-001",
    };
    const settled = await Promise.allSettled([
      c.funnel.promotePendingCapture({ captureId: first.capture.id, credential }),
      c.funnel.promotePendingCapture({ captureId: second.capture.id, credential }),
    ]);
    assert.equal(settled.filter(result => result.status === "fulfilled").length, 1);
    ...
    assert.equal(w.inspect.promotionCalls.length, 1);
    assert.equal((await w.storage.list("promotion-claims/")).length, 1);
    assert.equal((await c.funnel.list()).filter(row => row.promotion).length, 1);
    resetClock();
  });

  test("26. promoted erasure retains its retry handle after a CRM bridge failure", async () => {
    setClock(() => T0);
    const w = buildWorld();
    const c = container(w);
    const captured = await c.funnel.captureHcCompletion({
      email: "promotion-erasure-retry@example.com",
      completionId: "promotion_erasure_retry_01",
      slot: { slot: 5 },
    });
    w.inspect.authorityRecords.set("authenticated:erasure-session-001", {
      agencyId: AGENCY,
      installId: INSTALL,
      captureId: captured.capture.id,
      captureEmail: captured.capture.email,
      grant: {
        kind: "authenticated",
        operationId: "erasure-command-001",
        actorUserId: "agency_owner_001",
      },
    });
    await c.funnel.promotePendingCapture({
      captureId: captured.capture.id,
      credential: {
        kind: "authenticated",
        sessionToken: "erasure-session-001",
        operationId: "erasure-command-001",
      },
    });
    w.inspect.erasureFailures.remaining = 1;
    await assert.rejects(
      () => c.funnel.eraseExactCapture(captured.capture.id),
      /injected_crm_erasure_failure/,
    );
    assert.ok((await c.funnel.list())[0]?.promotion, "failed CRM erasure discarded the retry handle");
    const retried = await c.funnel.eraseExactCapture(captured.capture.id);
    assert.equal(retried.erased, true);
    assert.ok(retried.recordsErased >= 5);
    assert.deepEqual(w.inspect.erasureCalls, [captured.capture.id, captured.capture.id]);
    assert.equal((await c.funnel.list()).length, 0);
    resetClock();
  });
});
