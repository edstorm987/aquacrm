...
export function IncomeSheet({ payments, otherIncome, invoices, clients, apiBase }: { payments: Payment[]; otherIncome: IncomeEntry[]; invoices: Invoice[]; clients: Client[]; apiBase: string }) {
  ...
  return <section className="mx-auto w-full max-w-6xl space-y-6 pb-12">
    ...
    <header className="flex flex-wrap items-end justify-between gap-4">
      <div><p className="text-xs font-semibold uppercase tracking-wide text-black/45">Finance</p><h1 className="mt-1 text-2xl font-semibold text-black/90">Income sheet</h1><p className="mt-1 text-sm text-black/55">Every payment received, whether it came through an invoice or another source.</p></div>
...
function RecordOtherIncome({ clients, apiBase, onClose }: { clients: Client[]; apiBase: string; onClose: () => void }) {
  ...
  return <Modal title="Add other income" onClose={onClose}><form className="grid gap-4" onSubmit={async event => {
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      ...
      <label className={labelClass}>Reference (optional)<input name="reference" placeholder="Bank or payment reference" className={inputClass} /></label>
    </div>
...
function IncomeDetail({ row, onClose }: { row: IncomeRow; onClose: () => void }) {
  return <Modal title="Income details" onClose={onClose}><dl className="grid gap-0 divide-y divide-black/10 border-y border-black/10">{row.title ? <Detail label="Income" value={row.title} strong /> : null}<Detail label="Amount" value={money(row.amountCents, row.currency)} strong /><Detail label="Received" value={date(row.paidAt)} /><Detail label="Client" value={row.clientName} /><Detail label="Invoice" value={row.invoiceId ? row.invoiceNumber : "Not invoice-linked"} />{row.category ? <Detail label="Category" value={row.category} /> : null}<Detail label="Method" value={label(row.method)} /><Detail label="Reference" value={row.externalRef || "Not recorded"} />{row.description ? <Detail label="Description" value={row.description} /> : null}<Detail label="Notes" value={row.notes || "No notes"} /><Detail label={row.paymentId ? "Payment ID" : "Record ID"} value={row.paymentId || row.id} /></dl>{row.invoiceId ? <a href={`/portal/agency/agency-finance/invoices/${row.invoiceId}`} className="mt-5 inline-flex min-h-10 items-center rounded-md bg-black px-4 text-sm font-semibold text-white">Inspect invoice</a> : null}</Modal>;
}

function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) { return <div className="fixed inset-0 z-[90] grid items-end bg-black/35 sm:items-center sm:p-6"><button className="absolute inset-0" aria-label="Close" onClick={onClose} /><section role="dialog" aria-modal="true" aria-label={title} className="relative mx-auto max-h-[92vh] w-full max-w-xl overflow-y-auto bg-white p-5 shadow-2xl sm:rounded-lg sm:p-6"><header className="mb-5 flex items-center justify-between"><h2 className="text-xl font-semibold">{title}</h2><button type="button" onClick={onClose} aria-label="Close" className="grid size-9 place-items-center rounded-md border border-black/10"><X size={16} /></button></header>{children}</section></div>; }
