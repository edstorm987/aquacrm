...
// payment ONCE (the server derives the record id from it — see the plugin's
// lib/idempotency.ts). Recording a genuinely separate payment means re-opening
// the form → a fresh key → allowed. Same helper shape as the payment + income
// modals; without a key those creates were the last unguarded money-out path.
  ...
  return <section className="mx-auto w-full max-w-7xl space-y-7 pb-12">
    ...
    {view === "people" ? <div className="space-y-8">
      <section>
        ...
        {!hrEnabled ? <p className="mt-4 border-l-2 border-amber-500 bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-900">HR is not enabled yet. Finance profiles still work independently; enabling HR later will add staff and department linking.</p> : <p className="mt-3 text-xs text-black/40">Names and departments can link to HR while compensation remains private to Finance administrators.</p>}
    ...
    {editingProfile ? <ProfileModal profile={editingProfile === "new" ? undefined : editingProfile} apiBase={apiBase} defaultCurrency={defaultCurrency} companies={companies} staff={staff} departments={departments} budgetPots={budgetPots} onClose={() => setEditingProfile(null)} onSaved={rememberProfile} /> : null}
    {editingPayment ? <PaymentModal payment={typeof editingPayment === "object" && editingPayment.id !== "new" ? editingPayment : undefined} presetProfileId={typeof editingPayment === "object" && editingPayment.id === "new" ? editingPayment.profileId : undefined} apiBase={apiBase} profiles={profiles} budgetPots={budgetPots} onClose={() => setEditingPayment(null)} onSaved={rememberPayment} /> : null}
  </section>;
}
...
function ObligationModal({ item, apiBase, defaultCurrency, companies, legalDocuments, budgetPots, onClose, onSaved }: { item?: FinanceObligation; apiBase: string; defaultCurrency: Currency; companies: CompanyOption[]; legalDocuments: LegalDocument[]; budgetPots: BudgetPot[]; onClose: () => void; onSaved: (row: FinanceObligation) => void }) { const [busy, setBusy] = useState(false); const [error, setError] = useState(""); return <Modal title={item ? "Edit financial obligation" : "Add financial obligation"} onClose={onClose}><form onSubmit={async event => { event.preventDefault(); setBusy(true); setError(""); const data = new FormData(event.currentTarget); const payload = { name: String(data.get("name") ?? ""), type: data.get("type") as FinanceObligationType, status: data.get("status") as FinanceObligationStatus, frequency: data.get("frequency") as FinanceObligationFrequency, owner: String(data.get("owner") ?? ""), provider: String(data.get("provider") ?? ""), reference: String(data.get("reference") ?? ""), linkedLegalDocumentId: String(data.get("linkedLegalDocumentId") ?? "") || null, budgetPotId: String(data.get("budgetPotId") ?? "") || null, currency: data.get("currency") as Currency, expectedCostCents: toCents(data.get("expectedCost")), coverageAmountCents: toCents(data.get("coverageAmount")), effectiveAt: dateNumber(data.get("effectiveAt")), nextDueAt: dateNumber(data.get("nextDueAt")), reminderAt: dateNumber(data.get("reminderAt")), coverageEndsAt: dateNumber(data.get("coverageEndsAt")), companyIds: data.getAll("companyIds").map(String), notes: String(data.get("notes") ?? "") }; try { const response = await fetch(`${apiBase}/obligations${item ? `?id=${encodeURIComponent(item.id)}` : ""}`, { method: item ? "PATCH" : "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) }); const result = await response.json() as { obligation?: FinanceObligation; error?: string }; if (!response.ok || !result.obligation) return setError(result.error ?? "Could not save obligation."); onSaved(result.obligation); } finally { setBusy(false); } }}><div className="grid gap-4 sm:grid-cols-2"><label className={`${labelClass} sm:col-span-2`}>Name<input name="name" required defaultValue={item?.name} className={inputClass} placeholder="Professional indemnity renewal" /></label><Select label="Type" name="type" value={item?.type ?? "annual-accounts"} options={OBLIGATION_TYPES} /><Select label="Status" name="status" value={item?.status ?? "upcoming"} options={[["upcoming", "Upcoming"], ["action-required", "Action required"], ["in-progress", "In progress"], ["completed", "Completed"], ["waived", "Waived"], ["archived", "Archived"]]} /><Select label="Frequency" name="frequency" value={item?.frequency ?? "annual"} options={[["one-off", "One-off"], ["monthly", "Monthly"], ["quarterly", "Quarterly"], ["annual", "Annual"], ["custom", "Custom"]]} /><Select label="Currency" name="currency" value={item?.currency ?? defaultCurrency} options={SUPPORTED_CURRENCIES.map(currency => [currency.code, currency.label])} /><Field label="Owner"><input name="owner" defaultValue={item?.owner} className={inputClass} placeholder="Ed or accountant" /></Field><Field label="Provider / counterparty"><input name="provider" defaultValue={item?.provider} className={inputClass} placeholder="Insurer, HMRC or auditor" /></Field><Field label="Reference"><input name="reference" defaultValue={item?.reference} className={inputClass} placeholder="Policy or filing reference" /></Field><Field label="Expected cost"><input name="expectedCost" type="number" min="0" step="0.01" defaultValue={amountInput(item?.expectedCostCents)} className={inputClass} /></Field><Field label="Insurance cover amount"><input name="coverageAmount" type="number" min="0" step="0.01" defaultValue={amountInput(item?.coverageAmountCents)} className={inputClass} /></Field><Field label="Effective date"><input name="effectiveAt" type="date" defaultValue={dateInput(item?.effectiveAt)} className={inputClass} /></Field><Field label="Next deadline / renewal"><input name="nextDueAt" type="date" defaultValue={dateInput(item?.nextDueAt)} className={inputClass} /></Field><Field label="Reminder"><input name="reminderAt" type="date" defaultValue={dateInput(item?.reminderAt)} className={inputClass} /></Field><Field label="Cover / term ends"><input name="coverageEndsAt" type="date" defaultValue={dateInput(item?.coverageEndsAt)} className={inputClass} /></Field><Select label="Linked legal document" name="linkedLegalDocumentId" value={item?.linkedLegalDocumentId ?? ""} options={[["", "No linked document"], ...legalDocuments.filter(document => document.status !== "archived").map(document => [document.id, document.title] as [string, string])]} /><Select label="Budget pot" name="budgetPotId" value={item?.budgetPotId ?? ""} options={[["", "No budget pot"], ...budgetPots.map(pot => [pot.id, `${pot.name} (${pot.currency.toUpperCase()})`] as [string, string])]} /></div><CompanyChecks companies={companies} selected={item?.companyIds} /><Field label="Notes"><textarea name="notes" rows={3} defaultValue={item?.notes} className={`${inputClass} py-2`} placeholder="Coverage, filing requirements, evidence needed and next action" /></Field>{error ? <p role="alert" className="mt-4 text-sm text-red-700">{error}</p> : null}<ModalActions busy={busy} onClose={onClose} /></form></Modal>; }

function ProfileModal({ profile, apiBase, defaultCurrency, companies, staff, departments, budgetPots, onClose, onSaved }: { profile?: CompensationProfile; apiBase: string; defaultCurrency: Currency; companies: CompanyOption[]; staff: FinanceStaffOption[]; departments: FinanceDepartmentOption[]; budgetPots: BudgetPot[]; onClose: () => void; onSaved: (row: CompensationProfile) => void }) { const [busy, setBusy] = useState(false); const [error, setError] = useState(""); return <Modal title={profile ? "Edit compensation profile" : "Add person or supplier"} onClose={onClose}><form onSubmit={async event => { event.preventDefault(); setBusy(true); setError(""); const data = new FormData(event.currentTarget); const departmentId = String(data.get("departmentId") ?? ""); const departmentName = departments.find(department => department.id === departmentId)?.name ?? String(data.get("departmentName") ?? ""); const payload = { staffId: String(data.get("staffId") ?? "") || null, name: String(data.get("name") ?? ""), email: String(data.get("email") ?? ""), payeeType: data.get("payeeType") as PayeeType, departmentId: departmentId || null, departmentName: departmentName || null, title: String(data.get("title") ?? ""), companyIds: data.getAll("companyIds").map(String), budgetPotId: String(data.get("budgetPotId") ?? "") || null, currency: data.get("currency") as Currency, rateBasis: data.get("rateBasis") as CompensationRateBasis, baseRateCents: toCents(data.get("baseRate")), unitsPerWeek: Number(data.get("unitsPerWeek") || 0) || undefined, payFrequency: String(data.get("payFrequency") ?? "monthly"), employerCostPercent: Number(data.get("employerCostPercent") || 0), annualBonusTargetCents: toCents(data.get("annualBonusTarget")), nextPayAt: dateNumber(data.get("nextPayAt")), contractStartsAt: dateNumber(data.get("contractStartsAt")), contractEndsAt: dateNumber(data.get("contractEndsAt")), status: data.get("status") as CompensationProfileStatus, notes: String(data.get("notes") ?? "") }; try { const response = await fetch(`${apiBase}/profiles${profile ? `?id=${encodeURIComponent(profile.id)}` : ""}`, { method: profile ? "PATCH" : "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) }); const result = await response.json() as { profile?: CompensationProfile; error?: string }; if (!response.ok || !result.profile) return setError(result.error ?? "Could not save compensation profile."); onSaved(result.profile); } finally { setBusy(false); } }}><div className="grid gap-4 sm:grid-cols-2"><Select label="Link HR staff member" name="staffId" value={profile?.staffId ?? ""} options={[["", "Independent Finance record"], ...staff.map(person => [person.id, `${person.name} - ${person.title}`] as [string, string])]} /><Select label="Payee type" name="payeeType" value={profile?.payeeType ?? "employee"} options={[["employee", "Employee"], ["director", "Director"], ["freelancer", "Freelancer"], ["contractor", "Contractor"], ["agency", "Agency / supplier"]]} /><Field label="Name"><input required name="name" defaultValue={profile?.name} className={inputClass} placeholder="Person or supplier name" /></Field><Field label="Email"><input name="email" type="email" defaultValue={profile?.email} className={inputClass} /></Field><Field label="Role / service"><input name="title" defaultValue={profile?.title} className={inputClass} placeholder="Developer, photographer, accountant" /></Field>{departments.length ? <Select label="Department" name="departmentId" value={profile?.departmentId ?? ""} options={[["", "No HR department"], ...departments.map(department => [department.id, department.name] as [string, string])]} /> : <Field label="Department / cost centre"><input name="departmentName" defaultValue={profile?.departmentName} className={inputClass} placeholder="Operations" /></Field>}<Select label="Currency" name="currency" value={profile?.currency ?? defaultCurrency} options={SUPPORTED_CURRENCIES.map(currency => [currency.code, currency.label])} /><Select label="Rate basis" name="rateBasis" value={profile?.rateBasis ?? "annual"} options={[["annual", "Annual salary"], ["monthly", "Monthly retainer"], ["hourly", "Hourly"], ["daily", "Day rate"], ["fixed", "Fixed / milestone"]]} /><Field label="Base rate"><input required name="baseRate" type="number" min="0" step="0.01" defaultValue={amountInput(profile?.baseRateCents)} className={inputClass} /></Field><Field label="Hours or days per week"><input name="unitsPerWeek" type="number" min="0" max="168" step="0.25" defaultValue={profile?.unitsPerWeek} className={inputClass} /></Field><Select label="Pay frequency" name="payFrequency" value={profile?.payFrequency ?? "monthly"} options={[["weekly", "Weekly"], ["fortnightly", "Fortnightly"], ["monthly", "Monthly"], ["quarterly", "Quarterly"], ["milestone", "Per milestone"]]} /><Field label="Employer costs (%)"><input name="employerCostPercent" type="number" min="0" max="200" step="0.1" defaultValue={profile?.employerCostPercent ?? 0} className={inputClass} /></Field><Field label="Annual bonus target"><input name="annualBonusTarget" type="number" min="0" step="0.01" defaultValue={amountInput(profile?.annualBonusTargetCents)} className={inputClass} /></Field><Field label="Next pay date"><input name="nextPayAt" type="date" defaultValue={dateInput(profile?.nextPayAt)} className={inputClass} /></Field><Field label="Contract starts"><input name="contractStartsAt" type="date" defaultValue={dateInput(profile?.contractStartsAt)} className={inputClass} /></Field><Field label="Contract ends / review"><input name="contractEndsAt" type="date" defaultValue={dateInput(profile?.contractEndsAt)} className={inputClass} /></Field><Select label="Budget pot" name="budgetPotId" value={profile?.budgetPotId ?? ""} options={[["", "No budget pot"], ...budgetPots.map(pot => [pot.id, `${pot.name} (${pot.currency.toUpperCase()})`] as [string, string])]} />{profile ? <Select label="Status" name="status" value={profile.status} options={[["active", "Active"], ["paused", "Paused"], ["ended", "Ended"], ["archived", "Archived"]]} /> : <input type="hidden" name="status" value="active" />}</div><CompanyChecks companies={companies} selected={profile?.companyIds} /><Field label="Notes"><textarea name="notes" rows={3} defaultValue={profile?.notes} className={`${inputClass} py-2`} placeholder="Terms, review date, benefits, invoice requirements or approval notes" /></Field>{error ? <p role="alert" className="mt-4 text-sm text-red-700">{error}</p> : null}<ModalActions busy={busy} onClose={onClose} /></form></Modal>; }

function PaymentModal({ payment, presetProfileId, apiBase, profiles, budgetPots, onClose, onSaved }: { payment?: CompensationPayment; presetProfileId?: string; apiBase: string; profiles: CompensationProfile[]; budgetPots: BudgetPot[]; onClose: () => void; onSaved: (row: CompensationPayment) => void }) { const [profileId, setProfileId] = useState(payment?.profileId ?? presetProfileId ?? profiles.find(profile => profile.status === "active")?.id ?? ""); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [idempotencyKey] = useState(freshIdempotencyKey); const profile = profiles.find(item => item.id === profileId); return <Modal title={payment ? "Edit people payment" : "Record people payment"} onClose={onClose}><form onSubmit={async event => { event.preventDefault(); setBusy(true); setError(""); const data = new FormData(event.currentTarget); const status = data.get("status") as CompensationPaymentStatus; const payload = { profileId, kind: data.get("kind") as CompensationPaymentKind, periodLabel: String(data.get("periodLabel") ?? ""), currency: profile?.currency, grossCents: toCents(data.get("gross")), employerCostCents: toCents(data.get("employerCost")), status, dueAt: dateNumber(data.get("dueAt")), paidAt: status === "paid" ? dateNumber(data.get("paidAt")) : null, budgetPotId: String(data.get("budgetPotId") ?? "") || null, reference: String(data.get("reference") ?? ""), notes: String(data.get("notes") ?? ""), ...(payment ? {} : { idempotencyKey }) }; try { const response = await fetch(`${apiBase}/payments${payment ? `?id=${encodeURIComponent(payment.id)}` : ""}`, { method: payment ? "PATCH" : "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) }); const result = await response.json() as { payment?: CompensationPayment; error?: string }; if (!response.ok || !result.payment) return setError(result.error ?? "Could not save payment."); onSaved(result.payment); } finally { setBusy(false); } }}><div className="grid gap-4 sm:grid-cols-2"><label className={labelClass}>Payee<select name="profileId" required value={profileId} disabled={Boolean(payment)} onChange={event => setProfileId(event.target.value)} className={inputClass}><option value="">Choose payee</option>{profiles.filter(item => item.status !== "archived").map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><Select label="Payment type" name="kind" value={payment?.kind ?? (profile?.payeeType === "freelancer" ? "freelancer-invoice" : profile?.payeeType === "contractor" || profile?.payeeType === "agency" ? "contractor-invoice" : "salary")} options={PAYMENT_KINDS} /><Field label="Period / milestone"><input name="periodLabel" defaultValue={payment?.periodLabel} className={inputClass} placeholder="August 2026" /></Field><Select label="Status" name="status" value={payment?.status ?? "planned"} options={[["planned", "Planned"], ["approved", "Approved"], ["paid", "Paid"], ["cancelled", "Cancelled"]]} /><Field label={`Gross amount${profile ? ` (${profile.currency.toUpperCase()})` : ""}`}><input required name="gross" type="number" min="0" step="0.01" defaultValue={amountInput(payment?.grossCents)} className={inputClass} /></Field><Field label="Employer costs / fees"><input name="employerCost" type="number" min="0" step="0.01" defaultValue={amountInput(payment?.employerCostCents)} className={inputClass} /></Field><Field label="Due date"><input required name="dueAt" type="date" defaultValue={dateInput(payment?.dueAt ?? profile?.nextPayAt ?? Date.now())} className={inputClass} /></Field><Field label="Paid date"><input name="paidAt" type="date" defaultValue={dateInput(payment?.paidAt)} className={inputClass} /></Field><Select label="Budget pot" name="budgetPotId" value={payment?.budgetPotId ?? profile?.budgetPotId ?? ""} options={[["", "No budget pot"], ...budgetPots.filter(pot => !profile || pot.currency === profile.currency).map(pot => [pot.id, pot.name] as [string, string])]} /><Field label="Reference"><input name="reference" defaultValue={payment?.reference} className={inputClass} placeholder="Payroll or invoice reference" /></Field></div><Field label="Notes"><textarea name="notes" rows={3} defaultValue={payment?.notes} className={`${inputClass} py-2`} /></Field>{error ? <p role="alert" className="mt-4 text-sm text-red-700">{error}</p> : null}<ModalActions busy={busy} onClose={onClose} /></form></Modal>; }
...
function paymentDraftFor(profile: CompensationProfile): CompensationPayment { return { id: "new", agencyId: profile.agencyId, profileId: profile.id, budgetPotId: profile.budgetPotId, kind: profile.payeeType === "freelancer" ? "freelancer-invoice" : profile.payeeType === "contractor" || profile.payeeType === "agency" ? "contractor-invoice" : "salary", currency: profile.currency, grossCents: 0, employerCostCents: 0, status: "planned", dueAt: profile.nextPayAt ?? Date.now(), createdBy: profile.createdBy, createdAt: Date.now(), updatedAt: Date.now() }; }
async function quickComplete(item: FinanceObligation, apiBase: string, remember: (row: FinanceObligation) => void) { const response = await fetch(`${apiBase}/obligations?id=${encodeURIComponent(item.id)}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ status: "completed", lastCompletedAt: Date.now() }) }); const result = await response.json().catch(() => null) as { obligation?: FinanceObligation } | null; if (result?.obligation) remember(result.obligation); }
