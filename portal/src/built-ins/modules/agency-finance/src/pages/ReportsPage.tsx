...
export default async function ReportsPage(props: PluginPageProps) {
  ...
  const [invoices, expenses, payments, refunds] = await Promise.all([
    c.invoices.list(),
    c.expenses.list(),
    c.payments.list(),
  ...
  const outputTax = snapshot.outputTaxCents;
  const inputTax = snapshot.inputTaxCents;
  ...
  const deductibleCosts = snapshot.deductibleCashExpenseCents;
  ...
  const receivables = summariseAging(
    invoices
      .filter(invoice => isCollectibleInvoiceStatus(invoice.status) && invoice.currency === currency)
      .map(invoice => ({ amountCents: invoiceOutstandingCents(invoice, payments, refunds), dueAt: invoice.dueAt }))
  ...
  return (
    <section className="mx-auto w-full max-w-6xl space-y-8 pb-12">
      ...
      <FinanceCurrencyNav active={currency} available={snapshot.availableCurrencies} path="/portal/agency/agency-finance/reports" label="Report currency" />

      <dl className="grid grid-cols-2 border-y border-black/10 lg:grid-cols-8">
        <Metric label="Gross receipts" value={money(snapshot.grossCashRevenueCents, currency)} />
        <Metric label="Refunds" value={money(snapshot.refundCents, currency)} />
        <Metric label="Cash after refunds" value={money(snapshot.cashRevenueCents, currency)} />
        <Metric label="Cash paid" value={money(snapshot.cashExpenseCents, currency)} />
        <Metric label="Cash net" value={money(snapshot.cashNetCents, currency)} />
        <Metric label="Invoiced revenue" value={money(snapshot.accrualRevenueCents, currency)} />
        <Metric label="Committed costs" value={money(snapshot.committedExpenseCents, currency)} />
        <Metric label="Accrual net" value={money(snapshot.accrualNetCents, currency)} />
      </dl>

      <div className="grid gap-8 lg:grid-cols-2">
        <section>
          <h2 className="text-base font-semibold text-black/85">Committed costs by category</h2>
        ...
        <section>
          ...
          <dl className="mt-3 divide-y divide-black/10 border-y border-black/10 text-sm">
            <Row label="Tax attributed to cash receipts" value={money(outputTax, currency)} />
            ...
            <Row label="Deductible cash costs" value={money(deductibleCosts, currency)} />
            <Row label="Expenses missing receipts" value={String(snapshot.missingReceiptCount)} />
            <Row label="Pending costs (not committed)" value={money(snapshot.pendingExpenseCents, currency)} />
          </dl>
        </section>
      </div>
      ...
      <section>
        ...
        <div className="mt-3 overflow-x-auto">
          <table className="w-full min-w-[560px] text-sm">
            <thead className="border-b border-black/10 text-left text-[11px] uppercase tracking-wide text-black/45"><tr><th className="py-2">Month</th><th className="py-2 text-right">Gross receipts</th><th className="py-2 text-right">Refunds</th><th className="py-2 text-right">Cash after refunds</th><th className="py-2 text-right">Costs</th><th className="py-2 text-right">Net</th></tr></thead>
            <tbody>
              {snapshot.monthly.map(month => (
                <tr key={`${month.year}-${month.month}`} className="border-b border-black/[0.07]">
                  ...
                  <td className="py-3 text-right font-mono text-black/55">{money(month.cashExpenseCents, currency)}</td>
                  <td className="py-3 text-right font-mono font-semibold">{money(month.cashNetCents, currency)}</td>
                </tr>
              ))}
            </tbody>
          </table>
