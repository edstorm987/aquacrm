...
export function getUserByLogin(login: string, scope?: UserLookupScope): ServerUser | null {
  const trimmed = login.trim();
  if (!trimmed) return null;
  if (trimmed.includes("@")) return getUser(trimmed, scope);

  const wanted = normUsername(trimmed);
  for (const user of Object.values(getState().users)) {
    if (user.username && normUsername(user.username) === wanted) return user;
  }
  return null;
}
...
export function verifyLoginPassword(
  ...
  const user = getUserByLogin(login, scope);
  if (!user) {
    verifyHash(password, DUMMY_HASH);
    return null;
  }
...
export function updateUser(
  ...
  mutate(state => {
    ...
    const roleOrScopeChanged =
      ...
      (patch.agencyId !== undefined && patch.agencyId !== stored.agencyId) ||
