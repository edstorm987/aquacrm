...
export interface AgencyWebsiteTelemetryWriteScope {
  ...
  siteId: string;
  host: string;
  keyClass: "public" | "agency-master" | "agency-website";
}
...
function agencyMasterMatchesTelemetryScope(
  state: ReturnType<typeof getState>,
  scope: AgencyWebsiteTelemetryWriteScope,
): boolean {
  if (scope.keyClass !== "agency-master") return false;
  const owners = Object.entries(state.agencyMasterTagKeys ?? {})
    .filter(([, key]) => key === scope.siteKey)
    .map(([agencyId]) => agencyId);
  const exactSources = Object.values(state.websiteSources ?? {}).filter(source =>
    owners.includes(source.agencyId) && source.host === scope.host
  );
  return exactSources.length === 1
    && exactSources[0]?.id === scope.siteId
    && exactSources[0]?.agencyId === scope.agencyId;
}
...
function agencyWebsiteMatchesTelemetryScope(
  ...
  if (scope.keyClass === "agency-master") return agencyMasterMatchesTelemetryScope(state, scope);
...
export function recordAgencyWebsiteTelemetry(
  ...
  const scopedProject = resolvedScope
    ? resolvedScope.siteKey === siteKey
      ? state.agencyWebsites[resolvedScope.agencyId]
        ?? (publicSite
          ? ensurePrimaryAgencyWebsite()
          : resolvedScope.keyClass === "agency-master"
            && agencyMasterMatchesTelemetryScope(state, resolvedScope)
            ? ensureAgencyWebsite(resolvedScope.agencyId)
            : null)
      : null
    : null;
  const project = resolvedScope
    ? scopedProject && agencyWebsiteMatchesTelemetryScope(getState(), scopedProject, resolvedScope) ? scopedProject : null
  ...
  const propertyId = publicSite
    ...
    : cleanText(input.propertyId, 120)
      || (resolvedScope?.keyClass === "agency-master" ? resolvedScope.siteId : undefined);
  ...
  mutate(state => {
    ...
    if (
      !stored
      || (!publicSite && resolvedScope?.keyClass !== "agency-master" && stored.telemetrySiteKey !== siteKey)
      || (resolvedScope && !agencyWebsiteMatchesTelemetryScope(state, stored, resolvedScope))
    ) return;
    writeAccepted = true;
