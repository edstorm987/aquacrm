import "server-only";
// Promoting a trading company into its own agency — PREVIEW ONLY.
//
...
// moves a record. `previewCompanyPromotion` is a pure read: it takes the
 ...
 */
export function previewCompanyPromotion(agencyId: string, companyId: string): CompanyPromotionPreview | null {
  ...
  if (company.promotedAgencyId) {
    warnings.push(
      `This company was already promoted to agency \`${company.promotedAgencyId}\`. A second promotion creates nothing.`,
    );
  }
  ...
  return {
    ...
    alreadyPromotedAgencyId: company.promotedAgencyId,
...
function collectStaffTagAmbiguity(
  ...
  out.push({
    ...
    recordIds: ids,
    why:
      "`ServerUser.companyIds` is a display label, not a membership. Granting the new agency to these people because of a tag would widen sessions retroactively — the promotion must never become a way to widen a session. Propose, do not grant.",
    suggested: "leave",
  });
}
