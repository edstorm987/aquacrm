// Promotion disposition map — what happens to EVERY collection in `PortalState`
// when a trading company is promoted into its own agency.
//
...
// collections: `src/lib/server/showcaseMode.ts:477-519` names 25 of 78, and
// `src/lib/server/demoSeed.ts:430-463` names 7 of 78. Both silently miss the
...
// imports beyond the state type. The preview (`./promoteCompany.ts`) reads it;
...
export const PROMOTION_DISPOSITION = {
  ...
  agencies: {
    ...
    keying: "agency-id",
    reason:
      "`bootstrapAgency` creates the new tenant's Agency row. The origin agency row is never touched — a promotion adds a tenant, it does not edit one.",
  },
  tradingCompanies: {
    disposition: "leave",
    ...
    keying: "own-id",
    reason:
      "The promoted brand record STAYS in the origin tenant, tombstoned with `promotedAgencyId`/`promotedAt`. The new agency IS the company now; the tombstone is what makes a second POST idempotent and keeps the origin's history explaining where the brand went.",
  },
