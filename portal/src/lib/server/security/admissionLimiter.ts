...
//   - Production never resolves the memory adapter, even if an unsafe memory
//     override is present. Canonical Supabase URL + secret configuration is
//     enough to select the durable authority, matching `nonceStore.ts`.
//
...
function durableConfigured(): boolean {
  ...
  if (explicit === "durable" || explicit === "postgres" || explicit === "supabase") return true;
  // Memory is a local/test adapter only. A production typo or unsafe override
  // must degrade closed through the durable adapter, never silently create one
  // independent counter per process.
  if (explicit === "memory") return process.env.NODE_ENV === "production";

  const portalBackend = (process.env.PORTAL_BACKEND ?? "").toLowerCase();
  if (portalBackend === "postgres" || portalBackend === "supabase") return true;
  if (process.env.DATABASE_URL) return true;
  if (resolveSupabaseUrl() && resolveSupabaseSecretKey()) return true;
