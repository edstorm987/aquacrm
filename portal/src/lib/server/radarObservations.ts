...
export function buildRadarObservations(input: RadarObservationInputs): RadarObservation[] {
  ...
  add("company", "overall-health", metric(company?.health.overall ?? null, company?.health.overall ?? null, scoreStatus(company?.health.overall), `${company?.health.overall ?? "No"}/100`, "80 or above", "/portal/agency/company", "company-health", companyConnected, "Combined executive health score.", now));
  add("company", "income-health", metric(company?.health.income ?? null, null, scoreStatus(company?.health.income), `${company?.health.income ?? "No"}/100`, "80 or above", "/portal/agency/company", "company-income", companyConnected, "Revenue pace health.", now));
  ...
  add("company", "revenue-gap", metric(company?.revenueGapCents ?? null, null, company?.revenueGapCents ? "warning" : company ? "healthy" : "unknown", money(company?.revenueGapCents, company?.actuals.currency), "0 gap", "/portal/agency/company", "company:revenue-gap", companyConnected, "Remaining revenue required this month.", now, undefined, company?.revenueGapCents));
  add("company", "objectives", metric(profile?.objectives.length ?? null, null, profile ? minimumStatus(profile.objectives.length, 1) : "unknown", String(profile?.objectives.length ?? 0), "At least 1 current objective", "/portal/agency/company", "company:objectives", companyConnected, "Strategic objective register.", profile?.updatedAt || now, profile?.objectives.length));
  add("company", "plans", metric(profile?.plans.filter(plan => !["complete", "paused"].includes(plan.status)).length ?? null, null, profile ? minimumStatus(profile.plans.length, 1) : "unknown", String(profile?.plans.length ?? 0), "At least 1 active plan", "/portal/agency/company", "company:plans", companyConnected, "Business plan register.", profile?.updatedAt || now, profile?.plans.length));
  add("company", "capacity", metric(capacityPercent, null, capacityPercent >= (profile?.capacity.hiringTriggerPercent ?? 85) ? "warning" : company ? "healthy" : "unknown", `${capacityPercent}%`, `Below ${profile?.capacity.hiringTriggerPercent ?? 85}%`, "/portal/agency/company", "company:capacity", companyConnected, "Estimated weekly delivery and sales demand against usable hours.", profile?.updatedAt || now, activeClients.length, capacityPercent, "lower"));
  add("company", "trading-companies", metric(tradingCompanies.length, null, "healthy", String(tradingCompanies.length), "Every operating company recorded", "/portal/agency/company", "company:trading-companies", true, "Trading company register.", newest(tradingCompanies.map(item => item.updatedAt)) ?? now, tradingCompanies.length));
  add("company", "direction-profile", metric(profile ? Number(Boolean(profile.mission)) + Number(Boolean(profile.vision)) + Number(profile.values.length > 0) : null, null, profile ? minimumStatus(Number(Boolean(profile.mission)) + Number(Boolean(profile.vision)) + Number(profile.values.length > 0), 3) : "unknown", profile ? `${Number(Boolean(profile.mission)) + Number(Boolean(profile.vision)) + Number(profile.values.length > 0)}/3` : "Not available", "Mission, vision and values complete", "/portal/agency/company", "company:direction", companyConnected, "Company direction and executive context.", profile?.updatedAt || now, 3));
  ...
  add("clients", "telemetry-coverage", percentMetric(activeClients.length ? Math.round(clientsWithTelemetry / activeClients.length * 100) : null, 100, "Active clients with reporting telemetry.", "/portal/agency/performance", "clients:telemetry", true, telemetry.totals.latestEventAt));
  add("clients", "telemetry-freshness", metric(telemetry.totals.latestEventAt ? now - telemetry.totals.latestEventAt : null, null, telemetry.totals.latestEventAt && now - telemetry.totals.latestEventAt <= 2 * DAY ? "healthy" : telemetry.totals.properties ? "warning" : "unknown", telemetry.totals.latestEventAt ? `${Math.round((now - telemetry.totals.latestEventAt) / HOUR)}h ago` : "No events", "Within 48h", "/portal/agency/performance", "clients:telemetry", telemetry.totals.properties > 0, "Latest client or agency property activity.", telemetry.totals.latestEventAt ?? now, telemetry.totals.properties, undefined, "lower"));
  add("clients", "production-errors", zeroTargetMetric(telemetry.properties.filter(property => property.clientId).reduce((total, property) => total + property.errors24h, 0), "Client production errors in 24 hours.", "/portal/agency/performance", "clients:errors", telemetry.totals.properties > 0, telemetry.totals.latestEventAt, 1, 5));
  ...
  add("delivery", "milestones", countMetric(milestones.length, null, "Tracked client milestones.", "/portal/agency/performance", "delivery:milestones", true, newest(milestones.map(item => item.updatedAt)), "higher"));
  add("delivery", "blocked-milestones", zeroTargetMetric(milestones.filter(item => item.status === "blocked").length, "Blocked delivery milestones.", "/portal/agency/performance", "delivery:milestones", true, newest(milestones.map(item => item.updatedAt)), 1, 4));
  add("delivery", "overdue-milestones", zeroTargetMetric(overdueMilestones, "Milestones beyond target date.", "/portal/agency/performance", "delivery:milestones", true, newest(milestones.map(item => item.updatedAt)), 1, 4));
  ...
  add("marketing", "traffic-24h", countMetric(telemetry.totals.pageviews24h, null, "Pageviews in the last 24 hours.", "/portal/agency/performance", "marketing:traffic", telemetry.totals.properties > 0, telemetry.totals.latestEventAt, "higher"));
  add("marketing", "traffic-7d", countMetric(telemetry.totals.pageviews7d, telemetry.totals.previousPageviews7d, "Pageviews in the current 7-day window.", "/portal/agency/performance", "marketing:traffic", telemetry.totals.properties > 0, telemetry.totals.latestEventAt, "higher"));
  add("marketing", "traffic-change", metric(trafficChangePercent, null, trafficChangePercent === null ? "unknown" : trafficChangePercent <= -50 ? "warning" : "healthy", trafficChangePercent === null ? "No comparison" : `${trafficChangePercent > 0 ? "+" : ""}${trafficChangePercent}%`, "Stable or growing", "/portal/agency/performance", "marketing:traffic", telemetry.totals.properties > 0, "Traffic change against the preceding 7 days.", telemetry.totals.latestEventAt ?? now, telemetry.totals.pageviews7d + telemetry.totals.previousPageviews7d, undefined, "higher"));
  add("marketing", "traffic-surges", countMetric(telemetry.totals.trafficSurges, null, "Properties with traffic above the surge guardrail.", "/portal/agency/performance", "marketing:surges", telemetry.totals.properties > 0, telemetry.totals.latestEventAt, "neutral"));
  add("marketing", "traffic-drops", zeroTargetMetric(telemetry.totals.trafficDrops, "Properties with a material traffic drop.", "/portal/agency/performance", "marketing:drops", telemetry.totals.properties > 0, telemetry.totals.latestEventAt, 1, 3));
  add("marketing", "form-submissions", countMetric(telemetry.totals.forms7d, null, "Tracked forms submitted in 7 days.", "/portal/agency/performance", "marketing:forms", telemetry.totals.properties > 0, telemetry.totals.latestEventAt, "higher"));
  add("marketing", "conversions", countMetric(telemetry.totals.conversions7d, null, "Tracked conversion events in 7 days.", "/portal/agency/performance", "marketing:conversions", telemetry.totals.properties > 0, telemetry.totals.latestEventAt, "higher"));
  add("marketing", "conversion-rate", percentMetric(telemetry.totals.pageviews7d ? Math.round(telemetry.totals.conversions7d / telemetry.totals.pageviews7d * 10_000) / 100 : null, 1, "Conversions as a share of pageviews.", "/portal/agency/performance", "marketing:conversion-rate", telemetry.totals.properties > 0, telemetry.totals.latestEventAt));
  ...
  add("compliance", "legal-register", countMetric(activeLegal.length, null, "Active legal register records.", "/portal/agency/company#legal", "compliance:legal", true, newest(activeLegal.map(document => document.updatedAt)), "higher"));
  add("compliance", "expired-records", zeroTargetMetric(activeLegal.filter(document => document.status === "expired" || Boolean(document.expiresAt && document.expiresAt < now)).length, "Expired legal records.", "/portal/agency/company#legal", "compliance:legal", true, newest(activeLegal.map(document => document.updatedAt)), 1, 2));
  add("compliance", "due-records", zeroTargetMetric(dueLegal.length, "Legal records due in 30 days.", "/portal/agency/company#legal", "compliance:legal", true, newest(dueLegal.map(document => document.updatedAt)), 3, 8));
  add("compliance", "action-required", zeroTargetMetric(activeLegal.filter(document => document.status === "action-required").length, "Legal records marked action required.", "/portal/agency/company#legal", "compliance:legal", true, newest(activeLegal.map(document => document.updatedAt)), 1, 3));
  add("compliance", "insurance", metric(activeLegal.filter(document => document.category === "insurance" && document.status === "active").length, null, activeClients.length && !activeLegal.some(document => document.category === "insurance" && document.status === "active") ? "warning" : "healthy", String(activeLegal.filter(document => document.category === "insurance" && document.status === "active").length), "Current insurance recorded where required", "/portal/agency/company#legal", "compliance:insurance", true, "Insurance register coverage.", newest(activeLegal.filter(document => document.category === "insurance").map(document => document.updatedAt)) ?? now));
  add("compliance", "contracts", countMetric(allContracts.length, null, "Client contract records.", "/portal/agency/company#legal", "compliance:contracts", true, newest(allContracts.map(item => timestamp(item))), "higher"));
  add("compliance", "contract-acceptance", zeroTargetMetric(operationalAlerts.filter(alert => alert.id.includes("contract-awaiting")).length, "Contracts awaiting acceptance beyond guardrail.", "/portal/agency/company#legal", "compliance:contracts", true, newest(operationalAlerts.map(alert => alert.occurredAt)), 1, 4));
  add("compliance", "tax-records", metric(activeLegal.filter(document => document.category === "hmrc").length, null, activeLegal.some(document => document.category === "hmrc") ? "healthy" : "watch", String(activeLegal.filter(document => document.category === "hmrc").length), "Tax records captured", "/portal/agency/company#legal", "compliance:tax", true, "HMRC and tax register coverage.", newest(activeLegal.filter(document => document.category === "hmrc").map(document => document.updatedAt)) ?? now));
  add("compliance", "policy-coverage", metric(activeLegal.filter(document => document.category === "policy").length, null, activeLegal.some(document => document.category === "policy") ? "healthy" : "watch", String(activeLegal.filter(document => document.category === "policy").length), "Required policies recorded", "/portal/agency/company#legal", "compliance:policies", true, "Policy register coverage.", newest(activeLegal.filter(document => document.category === "policy").map(document => document.updatedAt)) ?? now));
  ...
  add("compliance", "compliance-freshness", metric(activeLegal.length ? now - (newest(activeLegal.map(document => document.updatedAt)) ?? now) : null, null, activeLegal.length && now - (newest(activeLegal.map(document => document.updatedAt)) ?? 0) > 90 * DAY ? "warning" : activeLegal.length ? "healthy" : "unknown", activeLegal.length ? `${Math.round((now - (newest(activeLegal.map(document => document.updatedAt)) ?? now)) / DAY)}d ago` : "No review", "Reviewed within 90d", "/portal/agency/company#legal", "compliance:legal", true, "Age of latest legal record update.", newest(activeLegal.map(document => document.updatedAt)) ?? now, activeLegal.length, undefined, "lower"));
  ...
  add("development", "property-coverage", countMetric(telemetry.totals.properties, null, "Agency and client digital properties.", "/portal/agency/performance", "development:properties", true, telemetry.totals.latestEventAt, "higher"));
  add("development", "tag-coverage", percentMetric(telemetry.totals.expectedProperties ? Math.round(telemetry.totals.connectedTags / telemetry.totals.expectedProperties * 100) : null, 100, "Expected live properties reporting telemetry.", "/portal/agency/performance", "development:tags", true, telemetry.totals.latestEventAt));
  add("development", "tag-freshness", zeroTargetMetric(telemetry.totals.staleTags, "Live tags silent for more than 48 hours.", "/portal/agency/performance", "development:tags", telemetry.totals.properties > 0, telemetry.totals.latestEventAt, 1, 3));
  add("development", "heartbeat-health", metric(telemetry.totals.heartbeats24h, null, telemetry.totals.expectedProperties && telemetry.totals.heartbeats24h === 0 ? "warning" : telemetry.totals.properties ? "healthy" : "unknown", String(telemetry.totals.heartbeats24h), "At least 1 heartbeat per live property", "/portal/agency/performance", "development:heartbeats", telemetry.totals.properties > 0, "Heartbeat events in 24 hours.", telemetry.totals.latestEventAt ?? now, telemetry.totals.properties));
  add("development", "production-errors", zeroTargetMetric(telemetry.totals.errors24h, "Production errors in 24 hours.", "/portal/agency/performance", "development:errors", telemetry.totals.properties > 0, telemetry.totals.latestEventAt, 1, 5));
  add("development", "error-rate", percentMetric(telemetry.totals.pageviews24h ? Math.round(telemetry.totals.errors24h / telemetry.totals.pageviews24h * 10_000) / 100 : telemetry.totals.errors24h ? 100 : 0, 0, "Errors relative to pageviews.", "/portal/agency/performance", "development:errors", telemetry.totals.properties > 0, telemetry.totals.latestEventAt, true));
  add("development", "load-performance", metric(telemetry.totals.averageLoadMs ?? null, null, telemetry.totals.averageLoadMs && telemetry.totals.averageLoadMs > 5_000 ? "critical" : telemetry.totals.averageLoadMs && telemetry.totals.averageLoadMs > 2_500 ? "warning" : telemetry.totals.averageLoadMs ? "healthy" : "unknown", telemetry.totals.averageLoadMs ? `${telemetry.totals.averageLoadMs}ms` : "No sample", "2500ms or faster", "/portal/agency/performance", "development:performance", telemetry.totals.properties > 0, "Average tracked page load.", telemetry.totals.latestEventAt ?? now, telemetry.totals.properties, undefined, "lower"));
  add("development", "slow-properties", zeroTargetMetric(telemetry.totals.slowProperties, "Properties slower than 2500ms.", "/portal/agency/performance", "development:performance", telemetry.totals.properties > 0, telemetry.totals.latestEventAt, 1, 3));
  add("development", "deployments", countMetric(telemetry.properties.reduce((total, property) => total + property.events.filter(event => event.type === "deployment" && event.occurredAt >= now - 30 * DAY).length, 0), null, "Deployments recorded in 30 days.", "/portal/agency/development/website", "development:deployments", telemetry.totals.properties > 0, telemetry.totals.latestEventAt, "neutral"));
  add("development", "release-errors", zeroTargetMetric(telemetry.properties.reduce((total, property) => total + property.events.filter(event => event.type === "error" && event.release && event.occurredAt >= now - 7 * DAY).length, 0), "Errors linked to a release in 7 days.", "/portal/agency/development/website", "development:releases", telemetry.totals.properties > 0, telemetry.totals.latestEventAt, 1, 4));
  add("development", "monitoring-silence", zeroTargetMetric(telemetry.properties.filter(property => property.expectedLive && (!property.lastSeenAt || now - property.lastSeenAt > 2 * DAY)).length, "Live properties not reporting.", "/portal/agency/performance", "development:silence", telemetry.totals.properties > 0, telemetry.totals.latestEventAt, 1, 3));
  add("development", "telemetry-integrity", percentMetric(telemetry.totals.properties ? Math.round(telemetry.properties.filter(property => property.events.every(event => Number.isFinite(event.occurredAt) && Boolean(event.type))).length / telemetry.totals.properties * 100) : null, 100, "Properties with structurally valid retained events.", "/portal/agency/performance", "development:integrity", telemetry.totals.properties > 0, telemetry.totals.latestEventAt));
  ...
  add("team", "capacity-plan", metric(profile?.capacity.weeklyAvailableHours ?? null, null, profile ? "healthy" : "unknown", profile ? `${profile.capacity.weeklyAvailableHours}h` : "Not set", "Weekly hours configured", "/portal/agency/company", "team:capacity", Boolean(profile), "Weekly available capacity plan.", profile?.updatedAt ?? now));
  add("team", "capacity-pressure", metric(capacityPercent, null, capacityPercent >= (profile?.capacity.hiringTriggerPercent ?? 85) ? "warning" : "healthy", `${capacityPercent}%`, `Below ${profile?.capacity.hiringTriggerPercent ?? 85}%`, "/portal/agency/company", "team:capacity", Boolean(profile), "Estimated capacity utilisation.", profile?.updatedAt ?? now, activeClients.length, undefined, "lower"));
  add("team", "hiring-trigger", metric(capacityPercent, null, capacityPercent >= (profile?.capacity.hiringTriggerPercent ?? 85) ? "warning" : "healthy", `${capacityPercent}%`, `Hiring review at ${profile?.capacity.hiringTriggerPercent ?? 85}%`, "/portal/agency/company", "team:hiring", Boolean(profile), "Capacity against configured hiring trigger.", profile?.updatedAt ?? now, activeClients.length, undefined, "lower"));
  ...
  add("team", "objective-ownership", percentMetric(profile?.objectives.length ? Math.round(profile.plans.filter(plan => plan.owner).length / Math.max(1, profile.plans.length) * 100) : null, 90, "Plans with an accountable owner.", "/portal/agency/company", "team:objectives", Boolean(profile), profile?.updatedAt));
  ...
  add("systems", "telemetry-ingestion", countMetric(telemetry.properties.reduce((total, property) => total + property.events.length, 0), null, "Retained telemetry events across properties.", "/portal/agency/performance", "systems:telemetry", telemetry.totals.properties > 0, telemetry.totals.latestEventAt, "higher"));
