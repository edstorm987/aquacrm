...
import type { BrandPortfolioRow, BrandPortfolioSnapshot } from "@/lib/brandPortfolio";
...
export async function buildCommandIntelligenceSnapshot({
  ...
  const attributionPercent = radar.commercial.leadCount
    ...
    : null;
  const traffic7d = checkValue(radar, "marketing", "traffic-7d") ?? 0;
  const trafficMeasured = checkMeasured(radar, "marketing", "traffic-7d");
  ...
  const forms7d = checkValue(radar, "marketing", "form-submissions") ?? 0;
  const formsMeasured = checkMeasured(radar, "marketing", "form-submissions");
  ...
  const commercialIntelligence = buildCommercialIntelligence({
    ...
    pageviews: traffic7d,
    forms: forms7d,
    pageviewsMeasured: trafficMeasured,
    formsMeasured,
    now,
  });
  const kpis = [
    ...
    makeKpi({ id: "traffic-7d", rank: 13, label: "Website traffic in 7 days", shortLabel: "Traffic", domain: "marketing", format: "number", value: traffic7d, previousValue: trafficPrevious7d ?? undefined, display: trafficMeasured ? traffic7d.toLocaleString() : "—", target: "Stable or growing qualified traffic", detail: "Aqua Tag pageviews across monitored agency and client properties.", href: "/portal/agency/fulfilment/technical/performance", sourceId: "marketing:traffic", measuredAt: findCheck(radar, "marketing", "traffic-7d")?.lastSeenAt ?? now, sampleSize: radar.summary.monitoredProperties, status: statusFromCheck(findCheck(radar, "marketing", "traffic-7d")), evidence: [trafficMeasured ? `Previous 7d ${(trafficPrevious7d ?? 0).toLocaleString()}` : "No pageview reading yet — not a measured zero", `${radar.summary.monitoredProperties} properties watched`], familyId: "traffic-7d", plan: { baselineValue: 0, targetValue: trafficBaseline || null, cadence: "weekly", source: "Previous seven-day traffic baseline" } }, evidence),
