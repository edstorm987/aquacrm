...
export const METADATA_KEY_CONTRACTS: readonly MetadataKeyContract[] = [
  ...
  plain("contactedAt", "journey", "number — epoch ms when the person was actually reached; personal Radar quota evidence reads it instead of the log timestamp", "server/activity", "activity"),
  ...
  plain("userId", "files", "string — the person who owns a private My Tools icon; the route refuses reads, replacements and deletes by anyone else", "lib/server/chrome/userChromeLayout", "private-object-lifecycle"),
  plain("toolId", "files", "string — the My Tools card a private icon upload belongs to; paired with userId so only the owning person's card can read, replace or delete it", "app/api/portal/chrome/tools/[toolId]/icon", "private-object-lifecycle"),
