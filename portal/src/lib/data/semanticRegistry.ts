...
export const SEMANTIC_ENTITIES: readonly SemanticEntity[] = [
  ...
  {
    id: "clientErasureTombstone",
    label: "Client erasure tombstone",
    definition:
      "A contact-data-free but pseudonymous denial fact proving that one agency/client inbox ownership scope was erased. " +
      "It is not an archived client record: a database trigger uses it only to prevent a delayed or future writer from recreating that client link.",
    idRule: "Composite (agency_id, client_id); one immutable row per erased client scope.",
    tenancy: "agency",
    tenantFields: ["agency_id", "client_id (opaque ownership reference only)"],
    sourceOfTruth: "Supabase inbox_client_erasure_tombstones; written only by erase_client_inbox_data.",
    plane: "operational",
    provenance: "Created in the same locked transaction that erases the exact inbox identity chain.",
    timestamps: { created: "erased_at" },
    sensitivity: "personal",
    retention: "Retained while anti-resurrection protection is required. It contains no name, contact detail, message, or provider identity, but the opaque client id remains pseudonymous personal data and must stay access-restricted and covered by retention review.",
    relationships: [{ to: "client", kind: "references", via: "opaque erased client id; denial-only, never hydration" }],
  },
  {
    id: "inboxItem",
    label: "Inbox item",
