...
export function buildRadarObservations(input: RadarObservationInputs): RadarObservation[] {
  ...
  add("sales", "conversion-cycle", metric(commercial.medianConversionMs, null, commercial.convertedLeadCount < 3 ? "unknown" : commercial.medianConversionMs && commercial.medianConversionMs >= 90 * DAY ? "critical" : commercial.medianConversionMs && commercial.medianConversionMs >= 45 * DAY ? "warning" : "healthy", commercial.medianConversionMs === null ? "Still gathering" : duration(commercial.medianConversionMs), "Median below 45 days", "/portal/clients?view=journey", "commercial:lifecycle", commercial.available, "Median elapsed time from lead capture to client conversion.", commercial.latestActivityAt, commercial.convertedLeadCount, undefined, "lower"));
  ...
  add("marketing", "lead-source-attribution", metric(commercial.leadCount ? Math.round((commercial.leadCount - commercial.unattributedLeadCount) / commercial.leadCount * 1000) / 10 : null, null, lowerRateStatus(commercial.leadCount ? commercial.unattributedLeadCount / commercial.leadCount * 100 : null, commercial.leadCount, 3, 15, 40), commercial.leadCount ? `${Math.round((commercial.leadCount - commercial.unattributedLeadCount) / commercial.leadCount * 1000) / 10}%` : "Still gathering", "85% or above", "/portal/agency/marketing", "commercial:lifecycle", commercial.available, "Retained leads with a usable acquisition source.", commercial.latestActivityAt, commercial.leadCount, undefined, "higher"));
