...
export function buildRadarWatchdogChecks(input: {
  ...
  const rules: Array<{ id: string; label: string; lens: RadarRuleLens; status: RadarCheckStatus; detail: string; evidence: string[] }> = [
    ...
    { id: "zero-blindness", label: "Zero-blindness guardrail", lens: "threshold", status: blindChecks.length ? "critical" : "pass", detail: blindChecks.length ? "At least one check cannot confirm health, so this is flagged as a blind spot rather than a healthy pass." : "Every check has an observable evidence path in this scan.", evidence: [`${blindChecks.length} blind checks`, "Tolerance 0"] },
...
function propertyChecks(property: RadarTelemetryProperty, now: number): BusinessRadarCheck[] {
  ...
  return [
    sentinel(development, "connection", missingLiveTag ? "blind" : property.tagDeclared || property.lastSeenAt ? "pass" : "watch", missingLiveTag ? `${property.label} is expected to be live but has sent no telemetry, so its health cannot be seen — install or repair its tracking tag so the radar can observe it.` : `${property.label} telemetry is declared or reporting.`, [`Expected live ${property.expectedLive ? "yes" : "no"}`, `Tag declared ${property.tagDeclared ? "yes" : "no"}`]),
    ...
    sentinel(development, "resilience", missingLiveTag ? "blind" : property.tagDeclared && property.lastSeenAt && property.heartbeats24h > 0 ? "pass" : property.lastSeenAt ? "watch" : "blind", property.tagDeclared && property.lastSeenAt && property.heartbeats24h > 0 ? `${property.label} has declaration, event flow, and heartbeat evidence.` : `${property.label} lacks one or more independent observability proofs, so a healthy reading cannot be trusted — the evidence below names which proof (tag, event, or heartbeat) is missing to close the blind spot.`, [`Tag ${property.tagDeclared ? "declared" : "missing"}`, `Event ${property.lastSeenAt ? "seen" : "missing"}`, `Heartbeat ${property.heartbeats24h ? "seen" : "missing"}`]),
  ];
}
