...
// PortalPageRenderer. Looks up the element component in the registry and
// passes a renderChildren helper down so containers don't have to
// re-import the renderer.
//
// Lifted here in P1 from
// `.../website-editor/src/components/BlockRenderer.tsx`, which now re-exports
// it verbatim. It resolves components through `./registry` — the shared lookup
// the plugin fills on import — rather than importing the plugin's
// `blockRegistry` directly, because this module is shared by three surfaces and
// only one of them is a plugin.
//
...
import { getElementDefinition, getElementRenderer } from "./registry";
...
export interface BlockRendererProps {
  ...
  /** P2 — live data channel, forwarded unchanged to every element in the tree. */
  context?: ElementContext;
}

function BlockRenderer({ blocks, editorMode = false, themeId, splitTestGroups, context }: BlockRendererProps) {
...
function BlockNode({
  ...
  const externalRenderer = getElementRenderer(block.type);
  const def = getElementDefinition(block.type);
...
export function BlockTreeRenderer({ blocks, editorMode, themeId, splitTestGroups, context }: BlockRendererProps) {
  return (
    <BlockRenderer
      blocks={blocks}
      editorMode={editorMode}
