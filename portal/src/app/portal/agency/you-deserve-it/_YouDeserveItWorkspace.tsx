...
import { CalendarDays, ExternalLink, Gift, PackageCheck, Pencil, Plus, ReceiptText, Trash2, Wallet, X } from "lucide-react";
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-7">
      ...
      <dl className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Metric label="In progress" value={String(records.filter(record => !["delivered", "cancelled"].includes(record.status)).length)} icon={<Gift size={17} />} tone="violet" />
        <Metric label="Due in 14 days" value={String(dueSoon)} icon={<CalendarDays size={17} />} tone="amber" />
        <Metric label="Planned spend" value={money(plannedSpend)} icon={<Wallet size={17} />} tone="blue" />
        <Metric label="Actual spend" value={money(actualSpend)} icon={<ReceiptText size={17} />} tone="emerald" />
      </dl>
      ...
      {visible.length ? <div className="grid gap-2">
      ...
      </div> : <div className="mm-surface-card rounded-lg border border-dashed border-black/15 px-5 py-12 text-center"><span className="mm-area-icon mx-auto grid size-12 place-items-center rounded-lg"><PackageCheck size={21} /></span><p className="mt-4 font-semibold text-black/70">{view === "delivered" ? "Nothing delivered yet" : "Nothing planned yet"}</p><p className="mx-auto mt-1 max-w-md text-sm leading-6 text-black/45">Add a welcome pack, important date or spontaneous thank-you when the moment feels right.</p>{view !== "delivered" ? <button type="button" onClick={() => setDraft({ ...EMPTY })} className="mt-5 inline-flex min-h-10 items-center gap-2 rounded-md bg-black px-3 text-sm font-semibold text-white"><Plus size={15} />Plan something</button> : null}</div>}
