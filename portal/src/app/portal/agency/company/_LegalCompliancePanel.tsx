...
export function LegalCompliancePanel({ initialDocuments, canEdit }: { initialDocuments: LegalDocument[]; canEdit: boolean }) {
  ...
  async function retryPendingDeletion(document: LegalDocument) {
    setStatus(`Retrying deletion of “${document.title}”…`);
    const response = await fetch(`/api/portal/company/legal?id=${encodeURIComponent(document.id)}${document.deleteDetach ? "&detach=true" : ""}`, { method: "DELETE" }).catch(() => null);
    const result = await response?.json().catch(() => null) as { ok?: boolean; error?: string; detail?: string } | null;
    if (response?.ok && result?.ok) {
      setDocuments(current => current.filter(item => item.id !== document.id));
      setStatus(`“${document.title}” permanently deleted.`);
      return;
    }
    setDocuments(current => current.map(item => item.id === document.id
      ...
      : item));
    setStatus(result?.error ?? "Deletion is still queued for recovery.");
  }
  ...
  return <div className="mt-7 space-y-7">
    ...
    {visible.length ? <div className="overflow-x-auto"><table className="w-full min-w-[820px] text-sm"><thead className="border-b border-black/10 text-left text-[11px] uppercase tracking-wide text-black/40"><tr><th className="py-3">Document</th><th className="py-3">Type</th><th className="py-3">Counterparty / reference</th><th className="py-3">Deadline</th><th className="py-3">Status</th><th className="py-3 text-right">Actions</th></tr></thead><tbody>{visible.map(document => <tr key={document.id} data-resolution-record={document.id} className="scroll-mt-24 border-b border-black/[0.07]"><td className="py-3 pr-3"><p className="font-medium text-black/80">{document.title}</p><p className="mt-0.5 text-xs text-black/40">{document.deleteState ? "Sensitive details hidden during deletion recovery" : `${document.fileName} · ${fileSize(document.size)}`}</p></td><td className="py-3 capitalize text-black/55">{categoryLabel(document.category)}</td><td className="py-3 text-black/55">{document.deleteState ? "—" : [document.counterparty, document.reference].filter(Boolean).join(" · ") || "—"}</td><td className="py-3">{document.deleteState ? "—" : deadline(document, now)}</td><td className="py-3">{document.deleteState ? <span className={`rounded-full px-2 py-1 text-[10px] font-semibold ${document.deleteState === "delete-failed" ? "bg-red-50 text-red-700" : "bg-amber-50 text-amber-800"}`}>{document.deleteState === "delete-failed" ? "Delete failed" : "Deleting"}</span> : <Status value={effectiveStatus(document, now)} />}</td><td className="py-3 text-right">{document.deleteState ? (canEdit ? <button type="button" onClick={() => void retryPendingDeletion(document)} className="min-h-9 rounded-md border border-red-200 px-3 text-xs font-semibold text-red-700">Retry delete</button> : null) : <div className="inline-flex gap-1"><button title="Inspect" aria-label={`Inspect ${document.title}`} onClick={() => setSelected(document)} className="grid size-9 place-items-center rounded-md border border-black/10 text-black/45"><Eye size={15} /></button><a title="Open document" aria-label={`Open ${document.title}`} target="_blank" href={`/api/portal/company/legal/content?id=${encodeURIComponent(document.id)}`} className="grid size-9 place-items-center rounded-md border border-black/10 text-black/45"><Download size={15} /></a></div>}</td></tr>)}</tbody></table></div> : <div className="grid min-h-52 place-items-center border-y border-dashed border-black/15 text-center"><div><ShieldCheck size={28} className="mx-auto text-black/20" /><p className="mt-3 text-sm font-medium text-black/65">No legal records to show</p><p className="mt-1 text-xs text-black/40">Upload your first contract, insurance policy, letter or template.</p></div></div>}
