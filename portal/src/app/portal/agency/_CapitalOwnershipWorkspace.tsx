...
function OwnershipRegister({ capital, canEdit, saving, commit }: CapitalRegisterProps) {
  ...
  return <RegisterLayout eyebrow="Authoritative cap table" title="Who owns and controls the company" detail="Ownership and voting percentages are calculated from issued shares and class rights, so displayed control cannot drift from the retained register.">
    <RegisterPanel title="Share classes" detail="Authorised capital, nominal value and rights." icon={<Building2 size={17} />}>
      ...
      <div className="divide-y divide-white/10">{capital.shareClasses.map(item => { const issued = capital.shareholders.filter(holder => holder.status === "active" && holder.shareClassId === item.id).reduce((sum, holder) => sum + holder.shares, 0); const linked = dependantsOf(capital, item.id); return <DataRow key={item.id} title={item.name} meta={`${formatNumber(issued)} issued · ${formatNumber(item.authorisedShares)} authorised · ${money(item.nominalValueCents, capital.currency)} nominal`} status={issued > item.authorisedShares ? "Over-issued" : item.dividendEligible ? "Dividend eligible" : "No dividend right"} tone={issued > item.authorisedShares ? "critical" : "clear"} onEdit={canEdit ? () => editClass(item) : undefined} onDelete={canEdit && !linked.length ? () => void commit({ ...capital, shareClasses: capital.shareClasses.filter(record => record.id !== item.id) }, "Share class removed.") : undefined} deleteBlocked={canEdit && linked.length ? blockedDelete("share class", item.name, linked, "Move its holders to another class first") : undefined} />; })}</div>
    </RegisterPanel>
...
function GovernanceRegister({ capital, canEdit, saving, commit }: CapitalRegisterProps) {
  ...
  return <RegisterLayout eyebrow="Governance and authority" title="Board and shareholder decisions" detail="Retain resolutions, votes, effective dates, supporting documents and the capital records each decision authorised."><RegisterPanel title="Decision register" detail="Draft, approve and preserve the authority behind executive action." icon={<Scale size={17} />}>
    ...
    <div className="divide-y divide-white/10">{capital.decisions.slice().sort((a, b) => (b.approvedAt ?? b.meetingAt ?? 0) - (a.approvedAt ?? a.meetingAt ?? 0)).map(item => <DataRow key={item.id} title={item.title} meta={`${readable(item.kind)} · ${item.proposedBy || "No proposer"}${item.documentId ? ` · document ${item.documentId}` : " · evidence not linked"}`} value={item.votesForPercent === undefined ? undefined : `${item.votesForPercent}% for`} status={item.status} tone={item.status === "approved" ? "clear" : item.status === "rejected" || item.status === "superseded" ? "muted" : "warning"} detail={item.summary} onEdit={canEdit ? () => edit(item) : undefined} onDelete={canEdit && !dependantsOf(capital, item.id).length ? () => void commit({ ...capital, decisions: capital.decisions.filter(record => record.id !== item.id) }, "Executive decision removed.") : undefined} deleteBlocked={canEdit && dependantsOf(capital, item.id).length ? blockedDelete("decision", item.title, dependantsOf(capital, item.id), "Mark it \u201csuperseded\u201d instead, which keeps the authority trail") : undefined} />)}{!capital.decisions.length ? <RegisterEmpty icon={<ScrollText size={20} />} title="No decisions retained" detail="Add the first board or shareholder decision so capital actions have an explicit authority trail." /> : null}</div>
  </RegisterPanel></RegisterLayout>;
}
...
function ownershipRows(capital: CompanyCapitalPlan) {
  ...
  return capital.shareholders.map(holder => {
    ...
    return { holder, shareClass, linked: dependantsOf(capital, holder.id), ownershipPercent: percent(activeShares, totalShares), votingPercent: percent(activeShares * (shareClass?.votingRightsPerShare ?? 0), totalVotes) };
  });
}
...
function DataRow({ title, meta, value, status, tone, detail, onEdit, onDelete, deleteBlocked }: { title: string; meta: string; value?: string; status: string; tone: "critical" | "warning" | "clear" | "muted"; detail?: string; onEdit?: () => void; onDelete?: () => void; deleteBlocked?: string }) { return <article className="grid gap-3 px-4 py-3 md:grid-cols-[minmax(0,1fr)_auto_auto] md:items-center"><div className="min-w-0"><strong className="block truncate text-sm text-white/72">{title}</strong><span className="mt-1 block text-[9px] uppercase text-white/30">{meta}</span>{detail ? <p className="mt-2 max-w-3xl text-xs leading-5 text-white/38">{detail}</p> : null}</div>{value ? <strong className="text-sm tabular-nums text-[#f1dba9]">{value}</strong> : null}<div className="flex items-center justify-end gap-3"><span className={`text-[8px] font-semibold uppercase ${tone === "critical" ? "text-red-300" : tone === "warning" ? "text-amber-200" : tone === "clear" ? "text-[#68f5d0]" : "text-white/25"}`}>{status}</span>{onEdit ? <button type="button" onClick={onEdit} className="text-[8px] font-semibold uppercase text-[#62e8ff]">Edit</button> : null}{onDelete ? <button type="button" onClick={onDelete} aria-label={`Delete ${title}`} className="text-red-300/55 hover:text-red-200"><Trash2 size={13} /></button> : null}{!onDelete && deleteBlocked ? <BlockedDelete reason={deleteBlocked} /> : null}</div></article>; }

/**
 * A hard delete that would strand a live link is not offered at all — the row
 * says what still holds the record and how to retire it instead. The server
 * refuses the same save, so this is the honest control, not the only guard.
 */
function BlockedDelete({ reason }: { reason: string }) { return <span title={reason} aria-label={reason} className="max-w-[16rem] truncate text-[8px] font-semibold uppercase text-white/30">Linked · retire, do not delete</span>; }

function dependantsOf(capital: CompanyCapitalPlan, id: string): string[] {
  return [
    ...capital.shareholders.filter(item => item.shareClassId === id).map(item => `shareholder \u201c${item.name}\u201d`),
    ...capital.transactions.filter(item => item.shareholderId === id || item.shareClassId === id || item.approvalId === id).map(item => `capital movement \u201c${item.title}\u201d`),
    ...capital.dividends.filter(item => item.approvalId === id || item.allocations.some(allocation => allocation.shareholderId === id)).map(item => `distribution \u201c${item.title}\u201d`),
  ];
}

function blockedDelete(noun: string, label: string, dependants: string[], exit: string): string {
  return `\u201c${label}\u201d cannot be deleted as a ${noun} while ${dependants.length} ${dependants.length === 1 ? "record" : "records"} still name it: ${dependants.join(", ")}. ${exit}, or detach those records first.`;
}
