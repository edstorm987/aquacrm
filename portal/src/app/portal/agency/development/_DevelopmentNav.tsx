...
export function DevelopmentNav({ active }: { active: "toolkit" | "workflow" | "vault" | "website" | "systems" }) {
  return (
    <nav aria-label="Development workspace" className="flex gap-1 overflow-x-auto border-b border-black/10">
      {items.map(item => {
        const Icon = item.icon;
        const id = item.href.endsWith("/toolkit") ? "toolkit" : item.href.endsWith("/workflow") ? "workflow" : item.href.endsWith("/vault") ? "vault" : item.href.endsWith("/website") ? "website" : "systems";
