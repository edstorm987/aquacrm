...
import { ArrowUpRight, Bell, BookOpen, CalendarDays, Check, ChevronLeft, ChevronRight, Clock3, ExternalLink, List, LoaderCircle, Pencil, Plus, Repeat2, Save, Sparkles, Trash2, UserRound, X } from "lucide-react";
...
function CalendarView({ month, tasks, actions, team, onPrevious, onNext, onToday }: { month: Date; tasks: AgencyTask[]; actions: GeneratedAction[]; team: TeamMember[]; onPrevious: () => void; onNext: () => void; onToday: () => void }) {
  ...
  return <section>
    <div className="flex flex-wrap items-center justify-between gap-3 pb-3"><h2 className="text-lg font-semibold text-black/80">{month.toLocaleDateString("en-GB", { month: "long", year: "numeric" })}</h2><div className="flex gap-1"><button onClick={onPrevious} aria-label="Previous month" className="grid size-9 place-items-center rounded-md border border-black/10"><ChevronLeft size={16} /></button><button onClick={onToday} className="inline-flex min-h-9 items-center gap-1.5 rounded-md border border-black/10 px-3 text-xs font-medium"><CalendarDays size={13} /> Today</button><button onClick={onNext} aria-label="Next month" className="grid size-9 place-items-center rounded-md border border-black/10"><ChevronRight size={16} /></button></div></div>
