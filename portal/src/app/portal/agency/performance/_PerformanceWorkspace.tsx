...
import { Activity, AlertTriangle, BarChart3, Check, ChevronRight, CircleGauge, Code2, ExternalLink, FileCheck2, FolderOpen, Gauge, Globe2, HeartPulse, Image, LoaderCircle, MousePointerClick, Plus, Search, Server, Target, Trash2, UsersRound, X } from "lucide-react";
...
import {
  ...
  type ClientMilestoneDeletePayload,
  type ClientMilestoneWritePayload,
} from "@/lib/client/performanceMilestoneMutationPayload";
...
export function PerformanceWorkspace({
  ...
  // Parent-owned mutation sequencing for the three checked families. A
  // mutation takes a sequence number when it starts and its authoritative
  // collection is applied only if nothing newer has already been applied for
  // that client, so a slow response that lands after a client switch or a view
  // remount still updates the right client, while an older response can never
  // overwrite a newer applied snapshot. A refused later attempt never advances
  // the applied mark, so it cannot discard an earlier valid receipt.
  const startedSequences = useRef(new Map<string, number>());
  ...
  function beginMutation(family: "reports" | "experiments" | "milestones", clientId: string): number {
    const key = `${family}:${clientId}`;
    const sequence = (startedSequences.current.get(key) ?? 0) + 1;
    startedSequences.current.set(key, sequence);
    return sequence;
  }

  function applyIfLatest(family: "reports" | "experiments" | "milestones", clientId: string, sequence: number): boolean {
    const key = `${family}:${clientId}`;
    if (sequence <= (appliedSequences.current.get(key) ?? 0)) return false;
    appliedSequences.current.set(key, sequence);
    return true;
  }

  function updateMilestones(clientId: string, milestones: ClientMilestone[], sequence: number) {
    if (!applyIfLatest("milestones", clientId, sequence)) return;
    setClients(current => current.map(client => client.id === clientId ? { ...client, milestones } : client));
  }
  ...
  function updateReports(clientId: string, reports: MonthlyPerformanceReport[], sequence: number) {
    if (!applyIfLatest("reports", clientId, sequence)) return;
    setClients(current => current.map(client => client.id === clientId ? { ...client, reports } : client));
  }

  function updateExperiments(clientId: string, experiments: PerformanceExperiment[], sequence: number) {
    if (!applyIfLatest("experiments", clientId, sequence)) return;
    setClients(current => current.map(client => client.id === clientId ? { ...client, experiments } : client));
  }
    ...
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-7">
      ...
      {view === "tag" && selected ? (
        <AquaTagDashboard
          ...
          beginReportMutation={() => beginMutation("reports", selected.id)}
          onReportsChange={(reports, sequence) => updateReports(selected.id, reports, sequence)}
        />
      ) : selected ? (
        <ClientPerformance
          key={selected.id}
          ...
          adding={adding}
          setAdding={setAdding}
          beginExperimentMutation={() => beginMutation("experiments", selected.id)}
          beginMilestoneMutation={() => beginMutation("milestones", selected.id)}
          onExperiments={(experiments, sequence) => updateExperiments(selected.id, experiments, sequence)}
          onMilestones={(milestones, sequence) => updateMilestones(selected.id, milestones, sequence)}
          period={period}
        />
      ) : (
...
function ClientPerformance({ client, adding, setAdding, beginExperimentMutation, beginMilestoneMutation: beginMilestoneSequence, onExperiments, onMilestones, period }: { client: PerformanceClient; adding: boolean; setAdding: (value: boolean) => void; beginExperimentMutation: () => number; beginMilestoneMutation: () => number; onExperiments: (items: PerformanceExperiment[], sequence: number) => void; onMilestones: (items: ClientMilestone[], sequence: number) => void; period: 7 | 28 | 90 }) {
  const [productId, setProductId] = useState(client.products[0]?.id ?? "overview");
  const [milestoneMutationId, setMilestoneMutationId] = useState<string | null>(null);
  const milestoneMutationInFlight = useRef<string | null>(null);
  ...
  // One milestone write at a time per mounted client (the row and dialog
  // controls disable while it runs); the returned sequence is the parent's
  // apply-order guard for the receipt.
  function beginMilestoneMutation(operationId: string): number | null {
    if (milestoneMutationInFlight.current !== null) return null;
    milestoneMutationInFlight.current = operationId;
    setMilestoneMutationId(operationId);
    return beginMilestoneSequence();
  }

  function finishMilestoneMutation(operationId: string) {
    if (milestoneMutationInFlight.current !== operationId) return;
    milestoneMutationInFlight.current = null;
    setMilestoneMutationId(null);
  }
  ...
  return (
    <>
      ...
      <ExperimentsPanel
        ...
        beginMutationSequence={beginExperimentMutation}
        onExperimentsChange={onExperiments}
      />
      ...
      {client.scope === "client" ? <section>
        <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
          ...
          {client.scope === "client" ? <button type="button" disabled={milestoneMutationId !== null} onClick={() => { if (milestoneMutationInFlight.current === null) setAdding(true); }} className="inline-flex min-h-10 items-center gap-2 rounded-md bg-black px-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60"><Plus size={15} />Add milestone</button> : null}
        </div>
        {client.milestones.length ? <div className="divide-y divide-black/10 border-y border-black/10">{client.milestones.map(item => <MilestoneRow key={item.id} clientId={client.id} item={item} mutationDisabled={milestoneMutationId !== null} onMutationStart={beginMilestoneMutation} onMutationEnd={finishMilestoneMutation} onMilestones={onMilestones} />)}</div> : <Empty title="No milestones yet" detail="Add a custom target such as launch approved, 50 enquiries, first booking, or photography delivered." />}
      </section> : null}
      {adding && client.scope === "client" ? <NewMilestone clientId={client.id} onMutationStart={beginMilestoneMutation} onMutationEnd={finishMilestoneMutation} onClose={() => setAdding(false)} onCreated={(milestones, sequence) => { onMilestones(milestones, sequence); setAdding(false); }} /> : null}
    </>
  );
}
...
function MilestoneRow({
  ...
  clientId: string;
  item: ClientMilestone;
  mutationDisabled: boolean;
  onMutationStart: (operationId: string) => number | null;
  onMutationEnd: (operationId: string) => void;
  onMilestones: (items: ClientMilestone[], sequence: number) => void;
}) {
  const [busyAction, setBusyAction] = useState<"update" | "delete" | null>(null);
  const [operationError, setOperationError] = useState("");
  const operationInFlight = useRef(false);
  const operationId = `milestone:${item.id}`;

  async function save(patch: Partial<ClientMilestone>) {
    if (operationInFlight.current || mutationDisabled) return;
    const sequence = onMutationStart(operationId);
    if (sequence === null) return;
    operationInFlight.current = true;
    setBusyAction("update");
    setOperationError("");
    try {
      const payload = await checkedJsonMutation<ClientMilestoneWritePayload>(
        "/api/tenants/client-milestones",
        {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ action: "update", clientId, milestoneId: item.id, ...patch }),
        },
        {
      ...
      onMilestones(payload.milestones, sequence);
    } catch (error) {
      setOperationError(mutationErrorMessage(error, "The milestone could not be updated."));
    } finally {
      operationInFlight.current = false;
      setBusyAction(null);
      onMutationEnd(operationId);
    }
  }
  ...
  async function remove() {
    ...
    setOperationError("");
    try {
      const payload = await checkedJsonMutation<ClientMilestoneDeletePayload>(
        "/api/tenants/client-milestones",
        {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ action: "delete", clientId, milestoneId: item.id }),
        },
        {
          fallback: "The milestone could not be deleted.",
          validate: value => isClientMilestoneDeletePayload(value, clientId, item.id),
        },
      );
      onMilestones(payload.milestones, sequence);
    } catch (error) {
      setOperationError(mutationErrorMessage(error, "The milestone could not be deleted."));
    } finally {
      operationInFlight.current = false;
      setBusyAction(null);
      onMutationEnd(operationId);
  ...
  return (
    <div className={`py-4 ${mutationDisabled && !busy ? "opacity-50" : ""}`} aria-busy={busy} aria-disabled={mutationDisabled && !busy}>
      <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_180px_150px_auto] md:items-center">
        ...
        <label className="grid gap-1 text-[10px] font-medium uppercase text-black/40">Progress<input aria-label={`${item.title} progress`} type="range" min="0" max="100" step="5" value={item.progress} disabled={mutationDisabled} onChange={event => void save({ progress: Number(event.target.value), status: Number(event.target.value) === 100 ? "complete" : item.status === "complete" ? "in-progress" : item.status })} /><span className="text-xs font-semibold text-black/60">{item.progress}%</span></label>
        <select aria-label={`${item.title} status`} value={item.status} disabled={mutationDisabled} onChange={event => void save({ status: event.target.value as ClientMilestoneStatus })} className="min-h-10 rounded-md border border-black/15 bg-white px-2 text-xs">
        ...
        <button type="button" disabled={mutationDisabled} onClick={() => void remove()} aria-label={busyAction === "delete" ? `Deleting ${item.title}` : `Delete ${item.title}`} className="grid size-9 place-items-center rounded-md text-black/35 hover:bg-red-50 hover:text-red-700 disabled:cursor-not-allowed disabled:opacity-45">{busyAction === "delete" ? <LoaderCircle size={14} className="animate-spin" /> : <Trash2 size={14} />}</button>
      </div>
      {busyAction === "update" ? <p role="status" className="mt-3 inline-flex items-center gap-2 text-xs font-semibold text-black/50"><LoaderCircle size={13} className="animate-spin" />Saving milestone...</p> : null}
      {operationError ? <p role="alert" className="mt-3 rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">{operationError}</p> : null}
    </div>
  );
}

function NewMilestone({ clientId, onMutationStart, onMutationEnd, onClose, onCreated }: { clientId: string; onMutationStart: (operationId: string) => number | null; onMutationEnd: (operationId: string) => void; onClose: () => void; onCreated: (items: ClientMilestone[], sequence: number) => void }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const operationId = `milestone:create:${clientId}`;
    const sequence = onMutationStart(operationId);
    if (sequence === null) {
      setOperationError("Another milestone change is still finishing. Try again in a moment.");
      return;
    }
    operationInFlight.current = true;
    setBusy(true);
    ...
    // Trim after the cap as well: the server trims what it receives, so a cut
    // that lands on a space would otherwise store one character less than the
    // receipt expects.
    const title = String(data.get("title") ?? "").trim().slice(0, 160).trim();
    const description = String(data.get("description") ?? "").trim().slice(0, 1_000).trim() || undefined;
    const parsedTargetAt = target ? new Date(`${target}T12:00:00`).getTime() : undefined;
    const targetAt = parsedTargetAt !== undefined && Number.isFinite(parsedTargetAt) && parsedTargetAt > 0 ? parsedTargetAt : undefined;
    const selectedMetric: ClientMilestone["metric"] = metric === "pageviews" || metric === "visitors" || metric === "conversions" || metric === "search-clicks" ? metric : undefined;
    const roundedTargetValue = Math.round(targetValue);
    const selectedTargetValue = selectedMetric && Number.isFinite(roundedTargetValue) && roundedTargetValue > 0 ? roundedTargetValue : undefined;
    const autoTrack = Boolean(selectedMetric && selectedTargetValue);
    try {
      const payload = await checkedJsonMutation<ClientMilestoneWritePayload>(
        "/api/tenants/client-milestones",
        {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ action: "create", clientId, title, description, targetAt, metric: selectedMetric, targetValue: selectedTargetValue, autoTrack }),
        },
        {
          fallback: "The milestone could not be added.",
          validate: value => isClientMilestoneWritePayload(value, {
            clientId,
            fields: {
              title,
      ...
      onCreated(payload.milestones, sequence);
    } catch (error) {
      setOperationError(mutationErrorMessage(error, "The milestone could not be added."));
    } finally {
      operationInFlight.current = false;
      setBusy(false);
      onMutationEnd(operationId);
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/35 p-4" role="presentation"><form onSubmit={submit} role="dialog" ref={dialogRef} aria-modal="true" aria-labelledby="new-milestone-title" aria-describedby={operationError ? "new-milestone-error" : undefined} aria-busy={busy} className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-lg border border-black/10 bg-white p-5 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-xs font-semibold uppercase text-brand">Custom milestone</p><h2 id="new-milestone-title" className="mt-1 text-xl font-semibold">What does success look like?</h2></div><button type="button" onClick={onClose} disabled={busy} aria-label="Close" className="disabled:cursor-not-allowed disabled:opacity-45"><X size={18} /></button></div><div className="mt-5 grid gap-4"><label className="grid gap-1 text-xs font-medium">Title<input name="title" required autoFocus className="min-h-11 rounded-md border border-black/15 px-3 text-sm" placeholder="Reach 50 website enquiries" /></label><label className="grid gap-1 text-xs font-medium">What counts as complete?<textarea name="description" rows={3} className="rounded-md border border-black/15 px-3 py-2 text-sm" /></label><div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-1 text-xs font-medium">Track automatically<select name="metric" className="min-h-11 rounded-md border border-black/15 bg-white px-3 text-sm"><option value="">No · update manually</option><option value="pageviews">Website views</option><option value="visitors">Visitors</option><option value="conversions">Conversions</option><option value="search-clicks">Search clicks</option></select></label><label className="grid gap-1 text-xs font-medium">Target number<input name="targetValue" type="number" min="1" className="min-h-11 rounded-md border border-black/15 px-3 text-sm" placeholder="50" /></label></div><label className="grid gap-1 text-xs font-medium">Target date<input name="targetAt" type="date" className="min-h-11 rounded-md border border-black/15 px-3 text-sm" /></label></div>{operationError ? <p id="new-milestone-error" role="alert" className="mt-4 rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">{operationError}</p> : null}<div className="mt-5 flex justify-end gap-2"><button type="button" onClick={onClose} disabled={busy} className="min-h-10 px-3 text-sm disabled:cursor-not-allowed disabled:opacity-45">Cancel</button><button type="submit" disabled={busy} className="min-h-10 rounded-md bg-black px-4 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60">{busy ? "Adding..." : "Add milestone"}</button></div></form></div>;
}
