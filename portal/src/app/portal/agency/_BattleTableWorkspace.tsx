...
export function BattleTableWorkspace({ payload, intelligence, onOpenIntelligence, radarIncidents = [], initialSection = "warroom", initialScopeId = "ecosystem" }: { payload: BattleTablePayload; intelligence: CommandIntelligenceSnapshot; onOpenIntelligence: (kpiIds?: string[], scopeId?: string) => void; radarIncidents?: WarRoomIncident[]; initialSection?: BattleTableSection; initialScopeId?: string }) {
  ...
  // P5 (war-room feel): every drill-in section reads as a numbered station of
  // the same command surface, not a settings page floating on its own.
  const activeStationIndex = sections.findIndex(item => item.id === section);
  const activeStation = activeStationIndex >= 0 ? sections[activeStationIndex] : undefined;
  const stationCode = activeStationIndex >= 0 ? `ST-${String(activeStationIndex + 1).padStart(2, "0")}` : "";
  ...
  return <section className="aqua-battle-table relative min-h-[46rem] overflow-hidden border border-[#d7b56d]/30 bg-[#050b0e] text-white" aria-labelledby="battle-table-heading">
    ...
    <div id="battle-table-body" className="relative max-h-[calc(100dvh-13rem)] min-h-[38rem] overflow-y-auto">
      {section !== "warroom" ? <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 border-b border-[#d7b56d]/12 bg-[#050d11]/80 px-4 py-2 sm:px-6"><button type="button" onClick={() => selectSection("warroom")} className="inline-flex min-h-8 items-center gap-2 text-[9px] font-semibold uppercase text-[#62e8ff] hover:text-white"><Zap size={12} /> Back to the war room</button>{activeStation ? <span className="inline-flex items-center gap-2 border border-[#d7b56d]/22 bg-[#d7b56d]/[0.06] px-2.5 py-1 text-[9px] font-semibold uppercase text-[#f1dba9]">{activeStation.icon}{stationCode} · {activeStation.label}</span> : null}<span className="text-[9px] uppercase text-white/25">Planning station · {selectedScope.label}</span><span className="ml-auto hidden items-center gap-1.5 text-[8px] font-semibold uppercase text-white/22 lg:inline-flex"><span className="size-1.5 bg-[#68f5d0] shadow-[0_0_6px_#68f5d0]" aria-hidden="true" /> Feeds the war room live</span></div> : null}
