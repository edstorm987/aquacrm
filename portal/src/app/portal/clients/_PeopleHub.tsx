...
import { Building2, ClipboardPenLine, Columns3, FileSpreadsheet, Fingerprint, HeartPulse, List, Mail, Megaphone, Phone, Plus, Route, Save, Search, Settings, UserRound, UserSearch, UsersRound, X, type LucideIcon } from "lucide-react";
...
export type PeopleHubView = "clients" | "leads" | "journey" | "contacts" | "staff" | "identity";
...
export function PeopleHub({
  ...
  clients: HubClient[];
  contacts: HubContact[];
  initialView: PeopleHubView;
  allowedViews: readonly PeopleHubView[];
  ...
  const [view, setView] = useState<PeopleHubView>(initialView);
  ...
  // The server owns the outer acquisition strip. Keep this local view in sync
  // with URL navigation so Contacts -> Journey cannot leave two tab strips
  // claiming contradictory active destinations, and browser Back restores the
  // visible workspace instead of a stale client-only state.
  ...
  function selectView(nextView: PeopleHubView) {
    if (!allowedViews.includes(nextView)) return;
    setView(nextView);
    router.push(nextView === "clients" ? "/portal/clients" : `/portal/clients?view=${nextView}`);
    if (nextView !== "clients") {
      setNiche("");
      setClientStatus("all");
    }
    ...
    <div className={view === "journey" ? "w-full" : "mx-auto w-full max-w-7xl"}>
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-black/65">Trace campaigns and enquiries into categorised leads, contacts and clients without forcing every person to become a customer.</p>
        </div>
        {canManage ? (
          <div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:flex-wrap">
            {view === "contacts" || view === "leads" ? (
              <Link href="/portal/agency/scouting?import=1" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-black/15 bg-white px-3 text-sm font-medium text-black/75 hover:bg-black/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2">
                <FileSpreadsheet size={16} aria-hidden="true" /> Import spreadsheet
              </Link>
            ) : null}
            <button type="button" disabled={!hydrated} aria-busy={!hydrated || undefined} onClick={() => setAddingContact(true)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-black/15 bg-white px-3 text-sm font-medium text-black/75 hover:bg-black/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2 disabled:opacity-50">
              <Plus size={16} /> Add contact
            </button>
            <NewClientButton products={products} brands={brands} defaults={clientDefaults} customFields={clientCustomFields} />
          </div>
        ) : <span className="rounded-full bg-sky-50 px-3 py-1.5 text-xs font-semibold text-sky-700 ring-1 ring-inset ring-sky-200">Read-only access</span>}
      </header>
      ...
      <div className="mt-6 border-b border-black/10">
        ...
        <div className="flex gap-3 overflow-x-auto sm:gap-6" role="group" aria-label="People view">
          {allowedViews.includes("clients") ? <Tab active={view === "clients"} onClick={() => selectView("clients")} label="Clients" count={clients.length} icon={Building2} attentionPrefixHref="/portal/clients" /> : null}
          {allowedViews.includes("leads") ? <Tab active={view === "leads"} onClick={() => selectView("leads")} label="Leads" count={leadRows.length} icon={UserSearch} attentionPrefixHref="/portal/agency/pipelines/leads" /> : null}
          {allowedViews.includes("journey") ? <Tab active={view === "journey"} onClick={() => selectView("journey")} label="Journey" count={journeyRows.length} icon={Route} attentionPrefixHref="/portal/agency/pipelines" /> : null}
          {allowedViews.includes("contacts") ? <Tab active={view === "contacts"} onClick={() => selectView("contacts")} label="Contacts" count={clients.length + contactRows.length} icon={UserRound} /> : null}
          {canManage && allowedViews.includes("identity") ? <Tab active={view === "identity"} onClick={() => selectView("identity")} label="Identity review" count={identityReviews.filter(review => review.status === "pending").length} icon={Fingerprint} /> : null}
          {allowedViews.includes("staff") ? <Tab active={view === "staff"} onClick={() => selectView("staff")} label="Staff" count={staffCount} icon={UsersRound} /> : null}
        </div>
      </div>
      ...
      {view !== "journey" && view !== "identity" ? <div className="flex flex-wrap items-center gap-2 border-b border-black/10 py-4">
        ...
        {(query || niche || clientStatus !== "all" || brandFilter || serviceFilter || leadCategoryFilter) ? <button type="button" onClick={() => { setQuery(""); setNiche(""); setClientStatus("all"); setBrandFilter(""); setServiceFilter(""); setLeadCategoryFilter(""); }} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-black/15 px-3 text-sm text-black/65 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2"><X size={14} /> Clear</button> : null}
      </div> : null}
...
function Tab({ active, onClick, label, count, icon: Icon, attentionPrefixHref }: { active: boolean; onClick: () => void; label: string; count: number; icon: LucideIcon; attentionPrefixHref?: string }) {
  return (
    <button type="button" aria-current={active ? "true" : undefined} onClick={onClick} className={`relative inline-flex min-h-11 items-center gap-2 whitespace-nowrap rounded-sm text-sm font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-inset ${active ? "text-black" : "text-black/65 hover:text-black/80"}`}>
      <Icon size={15} aria-hidden="true" /><span>{label} <span className="ml-1 text-xs tabular-nums text-black/65">{count}</span></span><AttentionDot prefixHref={attentionPrefixHref} />
    ...
    <section className="mm-surface-card mt-7 overflow-hidden rounded-lg border">
      <div className="flex items-center justify-between border-b border-black/10 px-4 py-3 sm:px-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="rounded-full bg-black/[0.04] px-2 py-0.5 text-xs font-medium text-black/65">{count}</span>
        </div>
      </div>
  ...
  return (
    <div className={`mm-interactive-row grid min-h-16 grid-cols-[auto_minmax(0,1fr)] items-center gap-3 px-4 py-3 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:px-5 ${needsAttention ? "bg-red-50/35" : ""}`}>
      ...
      <div className="min-w-0">
        ...
        <p className="mt-0.5 truncate text-xs text-black/65">{client.ownerEmail || "No account email"} · Source: {sourceLabel(client.source)}{client.websiteUrl ? ` · ${client.websiteUrl}` : ""}</p>
      ...
      <div className="col-start-2 flex w-fit flex-wrap items-center gap-2 sm:col-start-auto">
        {canManage && needsAttention ? <Link href={`/portal/clients/${client.id}?tab=relationship`} title={healthDetail} className="inline-flex min-h-11 items-center rounded-md border border-red-200 bg-red-50 px-3 text-xs font-semibold text-red-700 hover:bg-red-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2">Review health</Link> : null}
        {canManage ? <Link
          ...
          className="grid size-11 place-items-center rounded-md border border-black/15 bg-white text-black/65 hover:bg-black/[0.04] hover:text-black focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2"
        >
          <Settings size={15} aria-hidden="true" />
        </Link> : null}
        <Link href={`/portal/clients/${client.id}`} title={`Open ${client.name} internal workspace`} className="inline-flex min-h-11 items-center rounded-md border border-black/15 bg-white px-3 text-xs font-semibold text-black/70 hover:bg-black/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2">Open workspace</Link>
      </div>
    </div>
  );
}
    ...
    <div className={`mm-interactive-row grid min-h-16 grid-cols-[auto_minmax(0,1fr)] items-center gap-3 px-4 py-3 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:px-5 ${needsAttention ? "bg-red-50/35" : ""}`}>
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {lead.company && lead.name ? <span className="truncate text-xs text-black/65">{lead.company}</span> : null}
        ...
        <p className="mt-0.5 truncate text-xs text-black/65">{lead.email}{lead.phone ? ` · ${lead.phone}` : ""} · Source: {sourceLabel(lead.source)}{lead.tags.length ? ` · ${lead.tags.join(", ")}` : ""}</p>
        {needsAttention ? <p className="mt-1 flex items-center gap-1.5 text-xs font-medium text-red-700"><HeartPulse size={12} aria-hidden="true" />Latest enquiry is awaiting a response</p> : lead.notes ? <p className="mt-1 truncate text-xs text-black/65">{lead.notes}</p> : null}
      </div>
      <div className="col-start-2 flex w-fit flex-wrap items-center gap-1.5 sm:col-start-auto">
        {onReview && pipelineLeadId && lead.phone ? <Link href={`/portal/agency/prospecting?lead=${encodeURIComponent(pipelineLeadId)}&mode=power-dialler`} title="Open protected call desk" aria-label={`Open call desk for ${lead.name || lead.email}`} className="grid size-11 place-items-center rounded-md border border-black/10 bg-white text-black/65 hover:bg-black/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2"><Phone size={15} /></Link> : null}
        {onReview && pipelineLeadId && lead.email ? <Link href={`/portal/agency/prospecting?lead=${encodeURIComponent(pipelineLeadId)}&mode=email`} title="Open protected email desk" aria-label={`Open email desk for ${lead.name || lead.email}`} className="grid size-11 place-items-center rounded-md border border-black/10 bg-white text-black/65 hover:bg-black/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2"><Mail size={15} /></Link> : null}
        {onReview ? <button type="button" onClick={() => onReview(lead)} title="Review lead notes" aria-label={`Review ${lead.name || lead.email}`} className="grid size-11 place-items-center rounded-md border border-black/10 bg-white text-black/65 hover:bg-black/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2"><ClipboardPenLine size={15} /></button> : null}
        {onReview ? <Link href={workspaceHref} className="inline-flex min-h-11 items-center rounded-md border border-black/15 bg-white px-3 text-xs font-semibold text-black/70 hover:bg-black/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2">Open workspace</Link> : null}
      </div>
    </div>
  );
}
...
function buildJourneyRows(clients: HubClient[], contacts: HubContact[]): JourneyRow[] {
  ...
  for (const contact of contacts) {
    ...
    const hasExactClient = clients.some(client => contact.recordKind === "lead"
      ? clientMatchesLead(client, contact)
      : clientMatchesContact(client, contact));
    if (hasExactClient) continue;
...
function ContactRow({ contact, onReview }: { contact: HubContact; onReview?: (contact: HubContact) => void }) {
  return (
    <div className="mm-interactive-row grid min-h-16 grid-cols-[auto_minmax(0,1fr)] items-center gap-3 px-4 py-3 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:px-5">
      <div className="grid size-10 place-items-center rounded-md bg-black/[0.04] text-black/65"><UserRound size={18} /></div>
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {contact.company && contact.name ? <span className="truncate text-xs text-black/65">{contact.company}</span> : null}
        ...
        <p className="mt-0.5 truncate text-xs text-black/65">{contact.email}{contact.phone ? ` · ${contact.phone}` : ""} · Source: {sourceLabel(contact.source)}{contact.tags.length ? ` · ${contact.tags.join(", ")}` : ""}</p>
        {contact.notes ? <p className="mt-1 truncate text-xs text-black/65">{contact.notes}</p> : null}
      </div>
      <div className="col-start-2 flex flex-wrap items-center gap-1 sm:col-start-auto">
        {onReview ? <button type="button" onClick={() => onReview(contact)} title="Review notes" aria-label={`Review ${contact.name || contact.email}`} className="grid size-11 place-items-center rounded-md border border-black/10 text-black/65 hover:bg-black/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2"><ClipboardPenLine size={15} /></button> : null}
        {onReview ? <Link href="/portal/agency/leads-pipeline/contacts" className="inline-flex min-h-11 items-center rounded-md border border-black/15 px-3 py-2 text-xs font-medium text-black/70 hover:bg-black/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2">Protected contact controls</Link> : null}
      </div>
    </div>
  );
}
  ...
  return (
    <section className="mt-7">
      ...
      <div className="mt-5 flex flex-wrap items-end justify-between gap-3 border-b border-black/10 pb-4">
        <div><h2 className="text-base font-semibold text-black/80">Relationship journey</h2><p className="mt-1 text-sm text-black/65">Follow people from campaign or form source into contact records and client accounts.</p></div>
        <div className="flex flex-wrap items-center gap-2">
          <div className="inline-flex rounded-md border border-black/15 bg-white p-1" role="group" aria-label="Journey layout">
            <button type="button" aria-pressed={mode === "list"} onClick={() => setMode("list")} className={`inline-flex min-h-11 items-center gap-1.5 rounded px-2.5 text-xs font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2 ${mode === "list" ? "bg-black text-white" : "text-black/65 hover:bg-black/[0.04]"}`}>
              <List size={14} /> List
            </button>
            <button type="button" aria-pressed={mode === "kanban"} onClick={() => setMode("kanban")} className={`inline-flex min-h-11 items-center gap-1.5 rounded px-2.5 text-xs font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2 ${mode === "kanban" ? "bg-black text-white" : "text-black/65 hover:bg-black/[0.04]"}`}>
              <Columns3 size={14} /> Kanban
            </button>
          </div>
          {canManage ? <Link href="/portal/agency/marketing" className="inline-flex min-h-11 items-center gap-2 rounded-md border border-black/15 px-3 text-xs font-medium text-black/70 hover:bg-black/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2"><Megaphone size={14} /> Open marketing</Link> : null}
        </div>
      </div>
      ...
      {rows.map(row => (
        <article key={row.id} className="grid gap-3 px-4 py-4 sm:px-5 lg:grid-cols-[minmax(0,1fr)_210px_150px_auto] lg:items-center">
          <div className="min-w-0">
            ...
            <p className="mt-1 truncate text-xs text-black/65">{row.subtitle}</p>
            {row.notes ? <p className="mt-1 truncate text-xs text-black/65">{row.notes}</p> : null}
          </div>
          <div><p className="text-[10px] font-semibold uppercase text-black/65">Trace source</p><p className="mt-1 truncate text-xs font-medium text-black/65">{sourceLabel(row.source)}</p></div>
          <div><p className="text-[10px] font-semibold uppercase text-black/65">Stage</p><p className="mt-1 text-xs font-medium text-black/65">{row.stage}</p></div>
          <div className="flex flex-wrap gap-1 lg:justify-end">
            {row.contact && onReview ? <button type="button" onClick={() => onReview(row.contact!)} className="min-h-11 rounded-md border border-black/10 bg-white px-3 text-xs font-semibold text-black/65 hover:bg-black/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2">Review</button> : null}
            {canManage || row.client ? <Link href={row.href} className="inline-flex min-h-11 items-center rounded-md bg-black px-3 text-xs font-semibold text-white hover:bg-black/85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2">Open</Link> : null}
          </div>
        </article>
      ))}
            ...
            <section key={column.id} aria-labelledby={`journey-column-${column.id}`} className="min-h-[420px] border-t-2 border-black/70 bg-black/[0.025] px-3 pb-3">
              <header className="min-h-[104px] border-b border-black/10 py-3">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="rounded-full bg-white px-2 py-0.5 text-xs font-semibold tabular-nums text-black/65 ring-1 ring-inset ring-black/10">{columnRows.length}</span>
                </div>
                <p className="mt-1 text-xs leading-5 text-black/65">{column.description}</p>
              </header>
              <div className="mt-3 grid gap-2.5">
                {columnRows.map(row => <JourneyKanbanCard key={row.id} row={row} onReview={onReview} canManage={canManage} />)}
                {!columnRows.length ? <p className="py-8 text-center text-xs text-black/65">No records here.</p> : null}
              </div>
            </section>
          );
        })}
    ...
    <article className="rounded-lg border border-black/10 bg-white p-3 shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="mt-1 line-clamp-2 min-h-8 text-xs leading-4 text-black/65">{row.subtitle || journeyStatusLabel(row.status)}</p>
        </div>
        <Route size={15} className="mt-0.5 shrink-0 text-brand" />
      </div>
      <dl className="mt-3 grid gap-2 border-t border-black/[0.07] pt-3 text-xs">
        <div className="flex items-center justify-between gap-3"><dt className="text-black/65">Stage</dt><dd className="truncate font-medium text-black/65">{row.stage}</dd></div>
        <div className="flex items-center justify-between gap-3"><dt className="text-black/65">Source</dt><dd className="truncate font-medium text-black/65">{sourceLabel(row.source)}</dd></div>
      </dl>
      ...
      <div className="mt-3 flex items-center justify-end gap-1.5">
        {row.contact && onReview ? <button type="button" onClick={() => onReview(row.contact!)} className="min-h-11 rounded-md border border-black/10 px-2.5 text-xs font-semibold text-black/65 hover:bg-black/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2">Review</button> : null}
        {canManage || row.client ? <Link href={row.href} className="inline-flex min-h-11 items-center rounded-md bg-black px-2.5 text-xs font-semibold text-white hover:bg-black/85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2">Open</Link> : null}
      </div>
    </article>
  );
}
...
function JourneyMetric({ label, value }: { label: string; value: number }) {
  return <div className="rounded-lg border border-black/10 bg-white p-4"><p className="text-[10px] font-semibold uppercase tracking-wide text-black/65">{label}</p><p className="mt-1 text-2xl font-semibold tabular-nums text-black/85">{value}</p></div>;
}
...
function ContactScratchpad({ contact, onClose, onSaved }: { contact: HubContact; onClose: () => void; onSaved: (contact: HubContact) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] grid items-end bg-black/40 sm:items-center sm:p-6">
      <button type="button" tabIndex={-1} aria-label="Close notes" className="absolute inset-0" onClick={onClose} />
      <form ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="contact-scratchpad-title" onSubmit={save} className="relative mx-auto max-h-[100dvh] w-full max-w-2xl overflow-y-auto rounded-t-lg bg-white p-5 shadow-2xl sm:max-h-[92dvh] sm:rounded-lg sm:p-6">
        <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wide text-brand">Manual review</p><h2 id="contact-scratchpad-title" className="mt-1 text-xl font-semibold">{contact.name || contact.company || contact.email}</h2><p className="mt-1 text-sm text-black/65">{contact.email} · Source: {sourceLabel(contact.source)}</p></div><button type="button" onClick={onClose} aria-label="Close" className="grid size-11 place-items-center rounded-md border border-black/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2"><X size={16} /></button></div>
        {contact.recordKind === "contact" ? (
          <label className="mt-5 grid gap-1 text-xs font-medium text-black/65">Relationship
        ...
        <label className="mt-4 grid gap-1 text-xs font-medium text-black/65">Scratchpad notes
        ...
        <div className="mt-4 rounded-md bg-black/[0.025] p-3 text-xs leading-5 text-black/65">
        ...
        <div className="mt-6 flex justify-end gap-2"><button type="button" onClick={onClose} className="min-h-11 rounded-md border border-black/15 px-4 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2">Cancel</button><button disabled={busy} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-black px-4 text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2 disabled:opacity-50"><Save size={15} />{busy ? "Saving..." : "Save notes"}</button></div>
      </form>
    </div>
  );
}
...
function Badge({ children }: { children: React.ReactNode }) {
  return <span className="rounded-full bg-black/[0.05] px-2 py-0.5 text-[10px] font-medium text-black/65">{children}</span>;
}

function Empty({ text }: { text: string }) {
  return <div className="py-12 text-center"><Building2 className="mx-auto text-black/20" size={24} /><p className="mt-2 text-sm text-black/65">{text}</p></div>;
}
...
function AddContactModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] grid items-end bg-black/40 sm:items-center sm:p-6">
      <button type="button" tabIndex={-1} aria-label="Close contact form" className="absolute inset-0" onClick={onClose} />
      <form
        ref={dialogRef}
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wide text-black/65">New relationship</p><h2 id="add-contact-title" className="mt-1 text-xl font-semibold">Add contact</h2></div><button type="button" onClick={onClose} aria-label="Close" className="grid size-11 place-items-center rounded-md border border-black/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2"><X size={16} /></button></div>
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <label className="grid gap-1 text-xs font-medium text-black/65 sm:col-span-2">Scratchpad notes
          ...
          <label className="grid gap-1 text-xs font-medium text-black/65 sm:col-span-2">Relationship
        ...
        {error ? <p role="alert" className="mt-4 text-sm text-red-700">{error}</p> : null}
        <div className="mt-6 flex justify-end gap-2"><button type="button" onClick={onClose} className="min-h-11 rounded-md border border-black/15 px-4 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2">Cancel</button><button disabled={busy} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-black px-4 text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16877f] focus-visible:ring-offset-2 disabled:opacity-50"><Save size={15} />{busy ? "Saving..." : "Save contact"}</button></div>
      </form>
    </div>
  );
}
...
function Input({ label, name, type = "text", placeholder, required }: { label: string; name: string; type?: string; placeholder?: string; required?: boolean }) {
  return <label className="grid gap-1 text-xs font-medium text-black/65">{label}<input name={name} type={type} placeholder={placeholder} required={required} className="min-h-11 rounded-md border border-black/15 px-3 text-sm" /></label>;
}
