...
function displayPhaseLabel(phase: PhaseRow) {
  return phaseDisplayLabels[phase.stage] ?? phase.label;
}
      ...
      <ol className="mt-3 grid grid-cols-2 gap-1 sm:grid-cols-3 md:grid-cols-6">
        {phases.map(p => {
          ...
          const palette =
            ...
            : p.state === "active" ? "border-brand bg-brand/10 text-black/90 font-semibold"
            ...
            <li key={p.id}>
              ...
              >
                <span className="flex w-full items-center justify-between gap-1">
                  <span className="truncate">{displayPhaseLabel(p)}</span>
      ...
      {expanded && (
        <div className="mt-4 rounded-md border border-black/10 bg-black/[0.02] p-3">
          <div className="flex items-baseline justify-between gap-2">
            <h3 className="text-sm font-medium text-black/85">{displayPhaseLabel(expanded)} deliverables</h3>
            {expanded.stage === currentStage && nextPhase && currentPhase && (
              <button
                ...
                className="rounded-md bg-brand px-3 py-1 text-xs font-semibold text-white shadow hover:opacity-90 disabled:opacity-40"
