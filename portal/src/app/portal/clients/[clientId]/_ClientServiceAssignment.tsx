...
import { Check, ChevronDown, LoaderCircle, PackageCheck, Settings2 } from "lucide-react";
...
export function ClientServiceAssignment({
  ...
  async function save() {
    ...
    try {
      const response = await fetch("/api/tenants/client-products", {
        ...
        body: JSON.stringify({ clientId, productIds: selectedIds }),
      });
  ...
  return <section id="service-assignment" className={`overflow-hidden rounded-md border ${initialProducts.length ? "border-black/10 bg-white" : "border-amber-300 bg-amber-50"}`}>
    <header className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <h2 className="mt-1 text-base font-semibold text-black/82">{initialProducts.length ? `${initialProducts.length} service${initialProducts.length === 1 ? "" : "s"} assigned` : "Assign a product or service"}</h2>
          <p className="mt-1 text-xs leading-5 text-black/48">{initialProducts.length ? initialProducts.map(product => product.name).join(" · ") : `Choose what ${clientName} bought before delivery begins. Their internal workspace and portal will adapt automatically.`}</p>
        </div>
      </div>
    ...
    {editing && canManage ? <div className="border-t border-black/10 bg-white p-4 sm:p-5">
      ...
      {options.length ? <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-black/10 pt-4"><p className="text-xs text-black/45">{selectedOptions.length ? `${selectedOptions.length} selected: ${selectedOptions.map(option => option.name).join(", ")}` : "No service selected. Saving will return this client to assignment required."}</p><button type="button" disabled={busy || !dirty} onClick={() => void save()} className="inline-flex min-h-10 items-center gap-2 rounded-md bg-black px-4 text-xs font-semibold text-white disabled:opacity-40">{busy ? <LoaderCircle size={14} className="animate-spin" /> : <Check size={14} />}{busy ? "Saving" : "Save assignments"}</button></div> : null}
    </div> : null}
  </section>;
}
