...
export function ClientNotesWorkspace({ clientId, initial, canManage = true }: { clientId: string; initial: Record<NoteKey, string>; canManage?: boolean }) {
    ...
    <details className="group border-y border-black/10" data-testid="client-private-notes">
      ...
      <section className="grid gap-5 border-t border-black/10 pb-6 pt-5">
        <header className="flex flex-wrap items-center justify-between gap-4">
          ...
          {canManage ? <button type="button" onClick={() => void save()} disabled={busy} className="inline-flex min-h-10 items-center gap-2 rounded-md bg-black px-4 text-sm font-semibold text-white disabled:opacity-45"><Save size={15} /> {busy ? "Saving..." : "Save notes"}</button> : <span className="rounded-full bg-sky-50 px-3 py-1.5 text-xs font-semibold text-sky-700">Read-only</span>}
        </header>
