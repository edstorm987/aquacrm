...
import { ArrowRight, CircleAlert, Clock3, FolderKanban, PackageCheck, PanelTop } from "lucide-react";
...
export function ClientDeliveryOverview({ clientId, products, milestones, portalReady }: {
  clientId: string;
      ...
      <div className="grid gap-px overflow-hidden border border-black/10 bg-black/10 lg:grid-cols-[minmax(0,1.4fr)_minmax(280px,0.6fr)]">
        <div className="bg-white p-5">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {products.map(product => (
              <article key={product.id} className="border-l-2 border-brand bg-black/[0.018] px-3 py-3">
        ...
        <aside className="bg-white p-5">
          ...
          <ul className="mt-4 divide-y divide-black/[0.07]">
            {open.slice(0, 4).map(item => <li key={item.id} className="py-2.5"><p className="text-xs font-semibold text-black/68">{item.title}</p><p className="mt-0.5 text-[11px] text-black/40">{item.status}{item.targetAt ? ` · due ${new Date(item.targetAt).toLocaleDateString("en-GB")}` : ""}</p></li>)}
          </ul>
        </aside>
      </div>
