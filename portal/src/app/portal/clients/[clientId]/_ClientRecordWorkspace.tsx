    ...
    <section id="client-record" className="grid gap-5" data-testid="client-relationship-record">
      <header className="flex flex-wrap items-end justify-between gap-4 border-b border-black/10 pb-5">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {canManage ? <button type="button" onClick={() => setCaptureOpen(current => !current)} className="inline-flex min-h-10 items-center gap-2 rounded-md bg-black px-4 text-sm font-semibold text-white"><Plus size={15} /> Add record</button> : <span className="rounded-full bg-sky-50 px-3 py-1.5 text-xs font-semibold text-sky-700">Read-only record</span>}
        </div>
      </header>
                ...
                <div className="grid gap-6">
                  ...
                  {selectedItem.sourceType === "record-entry" ? (
                    <section className="border-t border-black/10 pt-5">
                      ...
                      {canManage ? <form onSubmit={event => { event.preventDefault(); void attachSelectedEvidence(selectedItem); }} className="mt-4 grid gap-3 border border-black/10 bg-black/[0.02] p-4">
          ...
          {timeline.map(item => (
            <li key={item.id} data-attention={item.attention} className={`grid gap-4 border-l-2 py-5 pl-3 lg:grid-cols-[2.5rem_minmax(0,1fr)_auto] lg:items-start ${item.attention === "critical" ? "border-l-red-500 bg-red-50/35" : item.attention === "warning" ? "border-l-amber-400 bg-amber-50/30" : !hasReliableDate(item.occurredAt) ? "border-l-amber-300" : "border-l-transparent"}`}>
              ...
              <div className="flex flex-wrap gap-1.5 lg:justify-end">
                ...
                {canManage && item.sourceType === "record-entry" ? <button type="button" disabled={busyId === item.sourceId} onClick={() => void updateEntry(item, item.visibility === "client" ? "internal" : "client")} className="inline-flex items-center gap-1.5 rounded-md border border-black/12 bg-white px-3 py-2 text-xs font-semibold text-black/58">{item.visibility === "client" ? <EyeOff size={13} /> : <Eye size={13} />}{item.visibility === "client" ? "Make private" : "Share"}</button> : null}
                {canManage && item.sourceType === "file" ? <button type="button" disabled={busyId === item.sourceId} onClick={() => void updateFileVisibility(item, item.visibility !== "client")} className="inline-flex items-center gap-1.5 rounded-md border border-black/12 bg-white px-3 py-2 text-xs font-semibold text-black/58">{item.visibility === "client" ? <EyeOff size={13} /> : <Eye size={13} />}{item.visibility === "client" ? "Make private" : "Share"}</button> : null}
                {canManage && item.sourceType === "record-entry" ? <button type="button" aria-label={`Remove ${item.title}`} disabled={busyId === item.sourceId} onClick={() => void deleteEntry(item)} className="grid size-9 place-items-center rounded-md border border-red-200 text-red-700"><Trash2 size={14} /></button> : null}
              </div>
            </li>
          ))}
        </ol>
