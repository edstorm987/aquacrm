...
// "+ Add system" picker. The runtime is modular internally, but this
// UI presents everything as built-in client systems.
  ...
  async function installLiveRecommended() {
    ...
    try {
      for (const pluginId of liveMissing) {
        ...
        if (!data.ok) {
          const copy = toolCopy({ id: pluginId, name: pluginId });
          setError(`${copy.name}: ${data.error ?? "Could not activate this system."}`);
          break;
        }
      }
        ...
        <aside className="flex flex-wrap items-start justify-between gap-3 rounded-lg border border-amber-300 bg-amber-50 p-3">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-amber-900/80">
              Typical live-stage system set: {liveRecommended.map(id => toolCopy({ id, name: id }).name).join(" · ")}.
            </p>
            {liveMissing.length === 0 ? (
              <p className="mt-1 text-[11px] text-amber-900/70">All recommended systems are already active.</p>
            ) : (
              <p className="mt-1 text-[11px] text-amber-900/70">
                {liveMissing.length} still to add: {liveMissing.map(id => toolCopy({ id, name: id }).name).join(", ")}.
              </p>
            )}
          </div>
          <button
          ...
          >
            {bulkRunning ? "Activating…" : "Activate recommended"}
          </button>
        </aside>
      )}
            ...
            <li key={p.id} className="flex items-start justify-between gap-3 rounded-lg border border-black/10 bg-white p-3">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {p.fromPreset && (
                    <span className="rounded-full bg-brand/10 px-2 py-0.5 text-[10px] uppercase tracking-wide text-brand">
                      stage system
                    </span>
                  )}
                </div>
                <div className="mt-0.5 text-xs text-black/55">{copy.description}</div>
              </div>
              <div className="flex shrink-0 items-center gap-1">
                {p.installed ? (
                  ...
                  <button
                    ...
                    className="rounded-md bg-brand px-3 py-1 text-xs font-medium text-white shadow hover:opacity-90 disabled:opacity-50"
                  >
                    + Add
                  </button>
                )}
