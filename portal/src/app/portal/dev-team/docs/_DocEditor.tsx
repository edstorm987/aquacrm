  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_18rem]">
      <div className="min-w-0">
        {changedOutsideApp ? (
          <div className="mb-3 flex items-start gap-2 rounded-xl border border-[#e7e0cd] bg-[#f6efdd] px-4 py-3 text-sm text-[#8a7228]">
        ...
        <textarea
          ...
          spellCheck={false}
          className="h-[62vh] w-full resize-y rounded-2xl border border-[color:var(--dt-line)] bg-[color:var(--dt-surface)] p-4 font-mono text-[12.5px] leading-relaxed text-[color:var(--dt-ink)] outline-none transition-colors focus:border-[#0b6f6d]"
        />
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <input
            ...
            maxLength={400}
            className="min-w-[14rem] flex-1 rounded-lg border border-[color:var(--dt-line)] bg-[color:var(--dt-surface)] px-3 py-2 text-sm text-[color:var(--dt-ink)] outline-none placeholder:text-[color:var(--dt-faint)] focus:border-[#0b6f6d]"
          />
          <button
            type="button"
            onClick={save}
        ...
        {error ? (
          <div className="mt-3 flex items-start gap-2 rounded-xl border border-[#eed3cf] bg-[#fbe9e6] px-4 py-3 text-sm text-[#a5443a]">
            <AlertTriangle size={16} className="mt-0.5 shrink-0" />
            <span>{error}</span>
          </div>
        ) : null}
        {saved && !error ? (
          <p className="mt-3 flex items-center gap-1.5 text-sm text-[#2f7d4f]">
                ...
                <li key={`${edit.at}-${i}`} className="py-2 first:pt-0 last:pb-0">
                  <div className="flex items-baseline justify-between gap-2">
                    <span className="flex items-center gap-1.5 text-sm font-medium text-[color:var(--dt-ink)]">
                      <UserRound size={12} className="text-[#0b6f6d]" />
                      {edit.author}
                    </span>
