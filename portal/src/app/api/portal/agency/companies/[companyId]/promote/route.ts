...
import { getTradingCompany, markTradingCompanyPromoted } from "@/server/tradingCompanies";
import { previewCompanyPromotion } from "@/server/promotion/promoteCompany";
import { getUserById } from "@/server/users";
import type { ServerUser, SessionPayload } from "@/server/types";

// Promote a trading company into its own agency — THE SECURITY SHELL.
//
...
//
// Idempotency: `TradingCompany.promotedAgencyId`. A second POST creates
...
export async function GET(request: NextRequest, context: PromoteRouteContext) {
  try {
    ...
    // `previewCompanyPromotion` returns null for "not in this agency" — the
    // same answer as "does not exist", which is why it becomes a plain refuse.
    const preview = previewCompanyPromotion(auth.agencyId, companyId);
...
export async function POST(request: NextRequest, context: PromoteRouteContext) {
  try {
    ...
    if (company.promotedAgencyId) {
      return noStore(
        NextResponse.json({
          ok: true,
          created: false,
          alreadyPromoted: true,
          agencyId: company.promotedAgencyId,
    ...
    try {
      ...
      markTradingCompanyPromoted(agencyId, companyId, agency.id, user.id);
      ...
      logActivity({
        ...
        category: "settings",
        action: "company.promoted",
        message: `${user.email} promoted trading company “${company.name}” into its own agency. No records moved.`,
        metadata: { companyId, promotedAgencyId: agency.id, recordsMoved: 0 },
      });
    } finally {
    ...
    const response = noStore(
      NextResponse.json({
        ...
        movedRecords: 0,
        note: "The tenant exists and you are a member of it. No records have moved — importing them is a separate, per-record-type step.",
      }),
    );
