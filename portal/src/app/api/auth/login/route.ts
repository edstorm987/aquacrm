...
// ("Milesymedia") plus an `agency-owner` user from the supplied
...
import { createUser, listUsersForAgency, verifyLoginPassword } from "@/server/users";
...
export async function POST(req: NextRequest) {
  ...
  if (agencies.length === 0) {
    ...
    const { agency } = await bootstrapAgency(
      { name: "Milesymedia", slug: "milesymedia", ownerEmail: email },
      provisional,
    );
  ...
  let user = embedClientId
    ? verifyLoginPassword(login, password, { clientId: embedClientId, role: "end-customer" })
    : null;
  if (!user) {
    user = verifyLoginPassword(login, password);
  }
  if (!user) {
