...
const MAX_FIELDS = 60;
const MAX_CAPTURE_BODY_BYTES = 160 * 1_024;
...
export async function POST(req: NextRequest) {
  ...
  const bounded = await readBoundedRequestBody(req, MAX_CAPTURE_BODY_BYTES);
  if (!bounded.ok) {
    return NextResponse.json(
      { ok: false, ...(bounded.status === 413 ? { error: "payload too large" } : {}) },
      { status: bounded.status, headers: corsHeaders(null) },
    );
  }
  const body = parseJsonObject(bounded.rawBody);
  if (!body) return NextResponse.json({ ok: false }, { status: 400, headers: corsHeaders(origin) });
