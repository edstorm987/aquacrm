...
import { embedCredentialAllows, revalidateEmbedCredential } from "@/lib/server/embedCredentialAuthority";
...
export default async function AquaAccountEmbed() {
  ...
  const client = session?.clientId
    ...
    : null;
  const credential = session?.embed && session.clientId
    ? revalidateEmbedCredential({
        agencyId: session.agencyId,
        clientId: session.clientId,
        credentialId: session.embed.credentialId,
        credentialVersion: session.embed.credentialVersion,
      })
    : null;
  if (!session || !client || !credential || !embedCredentialAllows(credential, client, "client")) {
