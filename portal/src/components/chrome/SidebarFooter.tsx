...
// configure anything. Scope layouts provide the correct settings destination,
// so client workspaces do not fall through to the agency settings hub.
...
export function SidebarFooter({ mobile = false, settingsItems = [] }: Props) {
  ...
  const settingsItem = accountItem
    ?? settingsItems.find(item => /settings$/i.test(item.label))
    ?? settingsItems[0];
  return (
    <div className={`mm-sidebar-footer flex shrink-0 flex-col gap-1 border-t border-black/10 pt-3 ${mobile ? "mt-3" : "mt-6"}`}>
      ...
      >
        ...
        <span className="mm-sidebar-link-label">{settingsItem?.label ?? "Settings"}</span>
      </Link>
