...
>
> Consolidated 2026-08-30 from **7** source documents / **85,076 words**. Each source is retained verbatim between provenance markers. The original path remains alongside it because relative links and runtime-backed Dev Team records still resolve from that location during the compatibility phase.
...
| Read-failure truth | **P1 — at least twenty-eight mounted reads turn unavailability into empty/default/stale data.** Website-source, communications/enquiry, invoice, contact, Meta, KPI, completed/evidence, Portal Editor, expense, commercial/manual-detail, resolution, search, Identity, phase-catalogue and governance-scope paths suppress failure. The UI can say none/no matches/current, omit data or controls, show the wrong queue/company/generic plan, or expose blank editors; sibling Finance can help render “Operations clear.” | Carry explicit availability/error through loaders and aggregates, refuse clear/health claims and editable defaults from missing evidence, expose retry and force failed reads through consequential mounted states. |
...
| Page performance / hidden read writes | **The bounded speed phase is complete, not a deployed performance claim.** Agency retains station-only construction, intent-loaded Search, suppressed development prefetch, an idle session monitor and static manifest. Its latest retained dev baseline was **3.8s compile + 315ms app cold / 784ms warm**. The static proxy import closure then fell **1,139,995→255,050B (-77.6%)**, but a concurrent external `tsconfig` alias prevented a clean post-change runtime comparison. Isolated production Agency/Dev Team fresh-process first HTTP was **727.8/726.4ms** and repeat max **28.3/31.2ms**. | 1280px Day/Battle/Library/Logs and 390×844 Logs/Agency/Battle are settled without overflow or console warnings/errors. Paused Day visibly remains unknown/not scanned rather than falsely clear. Completed links still carry `scan=1` and can rerun until replaced by a safe result token. Measure deployment geo/CDN/provider latency separately; continue hidden-read-mutation correctness work independently of speed. |
