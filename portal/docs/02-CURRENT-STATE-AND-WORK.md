...
| Published functional blocks | **Partially repaired.** Contact now uses a strict consent-bearing, origin-checked, rate-limited exact-install facade with one atomic receipt/submission record; its accepted version is bound to the digest of the exact displayed consent statement. Blog Feed reads published allowlisted summaries only. Both activate only on a real published mount, and focused visitor/backend proof is **27/27**. Ecommerce has its separate allowlisted storefront facade. The registry pins **341 total / 144 undeclared / 15 public routes**, with host/tenancy proof **50/50**. Forms, Booking/Reservations, Newsletter and Theme still name absent modules; Affiliate Leaderboard/Signup, Donation recurrence and other visitor promises retain their documented gaps. | Connect captured contacts to the intended operator inbox/notification flow, implement or remove the remaining absent-module controls, then browser-drive every published block on a real custom domain and live backend/provider. |
...
| Mutation error feedback | **Partial — the checked boundary, first broad cohort and all mounted Finance/Dev Team writes are implemented.** `checkedJsonMutation()` rejects transport, unreadable/malformed JSON, non-2xx, `{ok:false}` and invalid success payloads; 5xx bodies are opaque and unsafe/bloated 4xx/domain diagnostics fall back. All 22 Finance calls across ten components use strict ack/entity/multi-entity/HTTPS validators. Dev project save/delete/map/connect success is action- and expected-id-specific; exactly four remaining raw Dev fetches are pinned GET reads. | Focused checked-mutation proof is **25/25** and the independent combined behaviour rerun is **267/267**. The broader 148-family audit remains open: migrate the remaining Client Centre, Actions/Governance, phase, SOP, Company, Performance and related controls, then force 4xx/5xx/503, rejected-fetch and malformed responses through literal mounted browser flows before closing #47. |
...
| Read-failure truth | **P1 partly repaired.** Portal Editor configuration, attention plan/explanation/evidence, expense fields, KPI custom/shared views, completed history, Fulfilment phase catalogues, agency/exact-client website routing, Portal Search, Development search/resources, sender catalogues and Unified Inbox now distinguish unavailable from empty, retain labelled confirmed snapshots where safe, expose retry and lock dependent writes. Removed sender identities are cleared before ready; combined counts/empty copy are withheld over partial evidence. | Final interactive-read/utility proof is **14/14**. Customer/sibling Finance, contact-interaction, Meta, commercial/manual-detail, Identity and governance-scope families remain. Force rejection, lost response, retry and multi-tab/provider recovery through mounted acceptance before closing #57. |
...
| Responsive/accessibility verification | **Resolved as a repeatable local-browser gate.** `browser-matrix.mjs` drives real Chromium across 13 pages × 17 viewports, including all six primaries, 320×568, 200% zoom and both sides of every Tailwind breakpoint. Chromium **151.0.7922.34** completed all **1,326** required production checks as **1,175 passed / 0 failed / 151 observations / 0 missing**. Every observation was an explicitly proven aborted speculative Next RSC prefetch; none was an ordinary request failure. The fresh Settings six-primary slice is separately **36/36** with none. | Dialog/menu activation, screen-reader output, installability, forced errors, date boundaries and mutation journeys remain separate acceptance work. |
