...
**Why:** Authoritative by-id reads, stable completion ids and a global process transaction now collapse same-process replay and canonical-email races across configured installations. Anonymous completion is pending-only: it creates no User, session or CRM identity and emits only non-PII pending summaries. A server-only promotion command accepts only a foundation-validated capability and records exact capture-to-CRM lineage. The current host validates fresh signed agency sessions and fails caller-asserted mailbox proof closed because no durable receipt store or public proof endpoint exists yet.
...
**Why:** The 2026-08-25 identity/cookie behaviour is superseded by the 2026-09-12 ABUSE-002 boundary. Anonymous Health Check completion now persists only one pending Public Funnel capture, emits non-PII summaries and creates no User, session or CRM derivatives. A server-only idempotent promotion command accepts only a foundation-validated capability; the current host validates fresh signed agency sessions and fails mailbox proof closed until a durable receipt flow exists. There is no public promotion endpoint.
