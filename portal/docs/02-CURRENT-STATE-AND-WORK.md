...
>
> Consolidated 2026-09-02 from **7** source documents / **86,702 words**. Each source is retained verbatim between provenance markers. The original path remains alongside it because relative links and runtime-backed Dev Team records still resolve from that location during the compatibility phase.
...
- [`docs/development/status.md`](#source-docs-development-status-md) — 21,204 words · `11c7e3b6a3c1`
...
<!-- AQUACRM_SOURCE_START path="docs/development/status.md" sha256="11c7e3b6a3c1a4663fd8532a80a42ea02eb67fe61b7dd73c0861f11550ca9c02" -->
...
| Manifest plugin settings | **P2 partly repaired.** Shared settings are reachable across the registered families; Marketing, Website Editor, Fulfillment and Memberships now consume truthful retained fields, and unwired controls identify themselves at the input. Memberships applies its plan-trial default, safe billing-return target, member heading and fully gated annual cadence. The keyed source-derived inventory is **12 manifests / 40 fields: 30 consumed, 10 unwired** (later 2026-09-02: Finance's unenforced approval threshold, Ecommerce's unread publishable key and Leads Pipeline's unhonourable from-name removed; Ecommerce's low-stock threshold now the inventory default; Leads Pipeline's default source and capture-column label now consumed); it no longer mistakes an unrelated Leads CSV `defaultTags` key for a Client CRM settings consumer. | Focused Memberships **65/65**, adjacent **90/90** and changed-surface **145/145** pass; exact build `bcNH7NEvlzmp6z1VXtmch` browser-proves Memberships settings/controls within the **40/40** four-viewport matrix. `smoke-ecommerce-low-stock-default` **3/3**, `smoke-leads-pipeline-settings-consumers` **4/4** plus the manifest-pinning suites. Wire or remove the remaining HR (3), Public Funnel (2), Affiliates (2) and Client CRM (3) fields. |
