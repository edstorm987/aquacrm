...
>
> Consolidated 2026-09-01 from **7** source documents / **85,457 words**. Each source is retained verbatim between provenance markers. The original path remains alongside it because relative links and runtime-backed Dev Team records still resolve from that location during the compatibility phase.
...
- [`docs/development/status.md`](#source-docs-development-status-md) — 20,457 words · `8c7e6ee53978`
...
<!-- AQUACRM_SOURCE_START path="docs/development/roadmap.md" sha256="de9dbcb19413f51881f5af8fe87377fce29a1190811cca421c91dbbc611eca8b" -->
# Roadmap
...
<!-- AQUACRM_SOURCE_START path="docs/development/status.md" sha256="8c7e6ee539786141d3e8df05b7c2d814b84a21ab9589b92454057ee5c7c79570" -->
# Status & verification register

← Back to [development.md](../development.md) (the law)

> **Current checkpoint: [checklist.md](checklist.md).** This register explains
> verification depth and keeps dated feature evidence; it is not a second
> "where we stand" summary. The last documented whole-suite run, on 2026-08-23,
> is 3,621 pass / 0
> fail / 1 skipped, with the Postgres integration check skipped because
> `DATABASE_URL` was not supplied. A newer 2026-08-24 **non-security** sweep ran
> 3,428 tests after excluding the 13 explicitly auth/MFA/session-focused files:
> **3,428 pass / 0 fail / 1 Postgres skip across 620 suites**. Neither result is a
> substitute for the browser matrix in
> [ultra-review-2026-08-24.md](ultra-review-2026-08-24.md).
> A later 2026-08-25 selected non-security remediation gate ran **3,435 tests
> across 619 suites: 3,433 pass / 0 fail / 2 skipped**, with both skips tied to
> absent live-database configuration. TypeScript, `git diff --check` and the
> production build are green. The isolated Dev Team release candidate completed
> **268/268** static entries locally and again on Vercel. It is live at
> `aqua-crm.com`; Vercel's current production alias is the operational source
> for the deployment id.
>
...
- The last documented full test suite is green (**3,621 passing / 0 failing / 1 skipped**, measured 2026-08-23), but per the definition above that establishes **shape + pure-logic correctness**, not usability. The explicit skip is the live Postgres check without `DATABASE_URL`.
...
- The `.mjs` HTTP harnesses remain route/markup/runtime probes rather than browser interaction.
  In particular, `smoke-ux.mjs` still puts its three width labels only in the User-Agent. The new
  manual browser matrix closes the immediate evidence gap for representative visual layout and
  body overflow, but an automated responsive/accessibility gate is still required (#137).
...
for `/aqua-tag.js` were browser-proven. This does not prove the full cross-persona
grant/request mutation journey or every dynamic-module/freelancer-job/task association.
...
| Dev Team source-of-truth writes | **Partially repaired.** Filesystem-visible locks plus atomic replacement cover roadmap, Updates, thoughts and Findings; the standalone worker cooperates and finding create is exclusive. SHA-versioned doc saves reject stale processes and hash-bind attribution. Lock release/reaping atomically detaches the canonical directory before removal, closing the successor-lock ABA race; child-process/CAS proof and Dev cross-process **7/7** pass. | Make the document bytes and attribution ledger crash-coherent with a recoverable journal/transaction; inject death across that two-file commit and close or constrain the final check/rename window for direct writers that ignore the lock. Plan creation's `wx` path remains safe. |
...
| Payment-plan invoice recovery | **Implementation was resolved at the durable operation/adoption boundary, but its current fresh-process verification has drifted.** A recovery identity is persisted before Finance create; pending operations remain visible/retryable. | Shipment evidence was **4/4** and wider **119/119**. On 2026-08-26 two isolated reruns are **3/4**: the fresh-process request receives 422 under the changing revision contract. Reconcile that request/current revision before restoring the green claim, then complete mounted fault/retry acceptance. |
| Website Editor nested verification | **Resolved at the shared discovery-runner boundary.** Module `npm test`, root `smoke:website-editor` and canonical `smoke:all` now use/include one runner with portal path aliases and client React conditions. It discovers and attempts every file, retaining aggregate failure diagnostics. | Actual suite **49/49 files and 1,527 assertions**, fail-through contract **2/2**, TypeScript and isolated build **272/272** pass. The full root suite has unrelated concurrent failures, so no repository-wide green claim is made. Keep mounted editor behavior in the separate browser sweep. |
...
| Membership subscription lifecycle | **Code/behaviour resolved; mounted/live-provider acceptance pending.** One per-user cross-process command persists intent/provider result, uses stable provider identities and converges paid→paid, paid→free, free→paid and cancellation. Free cancellation is immediate instead of permanently pending. | Failure/retry/reload/concurrency and mounted-source proof **2/2**; widened Membership/customer/discount gate **49/49**, package+lifecycle **11/11**, TypeScript/diff pass. Production Stripe foundation #33 remains open, so browser/live-provider proof is not claimed. |
| Membership webhook delivery | **Code/behaviour resolved; signed live-provider acceptance pending.** A per-event scoped inbox retries failed/interrupted/legacy work and completes only after subscriber/payment state and synchronous side effects. Metadata/scope is required, payment rows are durable, event scope is real and processing failures return 503. | Fault/retry/fresh-container/concurrency/scope/payment proof **4/4**; combined Membership lifecycle+webhook **6/6**, widened **53/53**, package+dedicated **15/15**, TypeScript/diff pass. Production Stripe foundation #33 remains open. |
| Affiliate payout ownership | **Code/behaviour resolved; mounted/live-Connect acceptance pending.** Affiliate-scoped scheduling claims each approved attribution once and resumes partial row/index work. Manual/Stripe completion share a staged operation and reconcile earnings from canonical paid attributions, so retries and legacy duplicate payouts cannot double-count. | Fault/reload/concurrent schedule and completion proof **3/3**; package+focused **17/17**, combined Membership/Affiliate **70/70**, TypeScript/diff pass. Mounted Schedule exists; production Connect #45 and browser/live transfer remain. |
| Affiliate accounting truth | **Code/behaviour resolved; mounted/live-provider acceptance pending.** Eligible orders snapshot currency/settlement facts; payouts are currency-bound with gross/reversal/net composition, and cumulative cancellation/refund state is reconciled before transfer or as a same-currency future offset after settlement. | Mixed GBP/USD, pending-order exclusion, pre-payout cancellation, post-payout partial/full refund, replay and admin/affiliate source proof **3/3**; package+focused **20/20**, widened **79/79**, TypeScript/diff pass. Production Connect #45 and literal browser/provider proof remain. |
...
| Website editor route wiring | **P1 — several visible controls call absent, mismatched or stub APIs.** Funnels and Split have no registered server families. Publish/promote and ten `SitesPage` endpoint families use legacy paths; some handlers exist under `/api/portal/website-editor/*`, while schema/chatbot/settings remain absent or browser-local. The registered promote handler only returns pending and opens no PR. The AI probe hides only top-bar Generate; image variations/inpaint stay visible and call absent `ai-builder` routes. | Reconcile every request and visible capability with the real route table and durable outcome, implement or honestly remove unfinished controls, and add browser create/save/publish/promote/image-AI/reload coverage. |
| Published functional blocks | **P1 — multiple visitor controls are not operational or truthful.** Contact, Forms, Booking, Newsletter and Theme use absent paths; Blog/ecommerce/membership calls are role-gated; Affiliate Leaderboard hides an absent route as empty data, Affiliate Signup promises an email no code sends, and Donation's monthly selection still requests a one-off checkout. Registration/SSR tests do not exercise those outcomes. | Supply public tenant-aware endpoints or remove/label the controls; browser-drive each block and prove the intended durable record/side effect, including referral-code email and true recurring payment. |
| Website export | **P1 — the visible export control is unreachable and the dormant static renderer is incomplete.** Customise requests absent `/api/admin/export-code`; `handleExportSite()` is not registered; and a direct first-party Homepage render empties Hero, Product Grid, Testimonials and CTA. | Define one reachable export contract, preserve or visibly reject every publishable block, and test content/visual parity on representative first-party templates. |
| Website Editor admin data model | **P1 — even the main Sites station is a browser-local control plane.** Site creation, live/draft state, domains, primary selection, branding and custom code write `lk_sites_v1`, while host routing uses a separate server store. Sections and Popup have no storefront consumer; Customise values stay on that page; Page Detail is an unlinked local model with a `[pageId]`/`params.id` mismatch and no `/p/[slug]` route. | Remove the parallel stations or persist them through one canonical tenant/site/page model; prove shared state, real hostname routing and the actual shell/storefront effect after reload and from a second session. |
...
| Campaign email delivery | **P1 — “Send campaign” means enqueue, but is reported as sent.** The service queues outbox rows, stamps leads contacted and finalises the campaign as sent without calling provider delivery. Readiness is only `install.enabled`, and the page auto-installs/enables the plugin even when its provider is `none`. | Choose synchronous delivery or a durable queue worker; expose queued/sent/failed separately, tie lead contact stamps to the chosen milestone, and prove provider failure/retry end to end. |
| Paid Memberships | **P1 — impossible with the current foundation adapter.** `stripeFor()` always returns a throwing no-op object, so availability incorrectly reads true; paid seed rows silently disappear, all paid lifecycle operations fail, webhook verification is inert, and health still reports `ok:true`. Free-tier membership remains the only working billing mode. | Build the scoped real Stripe adapter from ecommerce configuration, make availability/health and seed outcomes truthful, then prove test-mode plan → checkout → signed webhook → portal/change/cancel. |
| Affiliate automated payouts | **P1 — Stripe Connect is not wired in production.** The plugin has onboarding, refresh, webhook and transfer services, but the live foundation omits `stripeConnect`; the visible customer setup route always answers not configured and no affiliate can become ready for the admin transfer button. Manual mark-paid remains available. | Supply a real client-scoped Connect adapter or gate/remove the automated controls, then prove account → hosted onboarding → webhook/status → transfer → completion in Stripe test mode. |
...
| Mutation error feedback | **Partial — the checked boundary and first cohort are implemented.** `checkedJsonMutation()` rejects transport, unreadable/malformed JSON, non-2xx, `{ok:false}` and invalid success payloads. Forty-six mutation calls across 17 mounted HR, Membership, Affiliate, Ecommerce, Finance Invoice, Task Template, Master Inbox and Team Workspace components now preserve retry context, settle pending state and show safe inline errors. | Dedicated helper/guard **5/5**, affected Team/People/Task/Notepad/Dashboard **109/109**, earlier cohort gates and TypeScript/diff pass. The broader 148-family audit remains open: migrate the remaining Finance, Client Centre, Actions/Governance, Development, phase, SOP, Company, Performance and related controls, then force 4xx/5xx/503, rejected-fetch and malformed responses through literal mounted browser flows before closing #47. |
...
| Existing-client phase transitions | **P1 — multi-step lifecycle changes are not atomic or resumable.** Plugin install changes happen before client stage/checklist/activity. A forced late activity-log failure rejected after the target stage and new plugin were already committed; skipped preset plugins and a failed starter variant can also ride inside overall success while both mounted controls discard the detail. | Persist one idempotent transition operation, reconcile/compensate every step, make partial/skipped state visible and fault-test every boundary through retry/reload. |
...
| Read-failure truth | **P1 partly repaired.** Portal Editor configuration, attention plan/explanation/evidence, expense fields, KPI custom/shared views, completed history and Fulfillment phase catalogues now distinguish unavailable from empty, retain confirmed snapshots, expose retry and lock unsafe writes. Completed deletion and custom-KPI creation also replay ambiguous outcomes without duplication. | Website-source, customer/sibling Finance, communications/contact, Meta, commercial/manual-detail, search, Identity and governance-scope families remain. Force rejection, lost response, retry and multi-tab/provider recovery through mounted acceptance before closing #57. |
| Utility action settling | **P2 — five non-mutating controls can hang or misreport after rejection.** Task Template load, Development pagination/credential reveal and Search Console checking can remain busy forever; Copy Tag writes the same value twice and can copy before withholding success. | Use one checked attempt per action, clear pending state in `finally`, expose unavailable/retry or copied/error and force fetch/clipboard rejection in component/browser coverage. |
...
| Email Sender setup | **P1 — the one real provider cannot be configured truthfully from the app.** Sender Settings is read-only; the manifest omits the required API key; Postmark is absent from shared integrations; provider and identity mutation APIs have no UI caller; and identity “verification” immediately marks any address active without a provider check. | Create one canonical encrypted provider/identity setup, real verification and explicit readiness states; browser-prove fresh install → credentials → verified sender → test delivery → webhook. |
| Manifest plugin settings | **P2 — most declared settings are not user-operable.** Twelve built-ins declare 51 fields, but only Finance mounts `PluginSettingsPanel`; several custom Settings pages are read-only, other modules expose none, and multiple fields have no runtime consumer. Email Sender also persists `defaultFromIdentityId` even though delivery ignores it and resolves the independently marked `isDefault` row. | Mount one scoped generic/custom editor per retained schema, remove dead/split fields, and prove each save survives reload and changes the intended behavior. |
...
| Private uploads | **P1 — binary storage and owning records can diverge across the platform.** Nine routes store first; six then create CRM/database records and three return browser-staged references with no abandonment cleanup. Client-file, legal, SOP and development deletes suppress storage errors after removing the record; a later record failure can also leave a broken link. The product-workspace batch silently caps at 30, overstates the completed count and can duplicate earlier files on partial-failure retry. | Add a shared durable pending/failed lifecycle, expire abandoned staged objects, preserve provider diagnostics and keys, reconcile/compensate partial operations, and prove provider plus mounted-batch failure/retry/reload across Supabase, Blob and local storage. |
...
| Data integrity | **Broad field-level reference work remains; the audited client-route and website slices are fixed.** Identity Resolution, Inbox, People, Dev Projects, Performance Experiments and Plugin Settings require a resolved scoped client; the focused route chain passes **55/55**. `readAgencyWebsite()` returns null and Marketing shows an unconfigured state. Memory/source checks still cover unresolved task/template, product, KPI, Custom AI, Development, People/Team Chat, Inbox Connection, Finance, HR, Marketing, Leads Pipeline, Client CRM, Memberships and Email Sender references, plus parent deletes that strand children. | Continue per-handler/per-field validation or explicit stale-reference policy for the remaining matrix; do not reopen the six fixed client-bearing routes or fabricated website default. |
...
| Membership/Affiliate retirement | **P1 — hard deletion strands active dependants.** A plan-delete probe left its subscription row but hid it from the admin list and removed benefits; an affiliate-delete probe left an active code, approved attribution and scheduled payout tied to the missing parent. Billing/destination reconciliation is absent. | Route retirement through archive/removed states or one explicit retention/cascade operation; preserve subscriber and financial history, reconcile external systems and fault-test active dependants, reload and retry. |
| SOP retirement | **P1 — permanent deletion breaks active operating links.** A fresh probe deleted one SOP while a guide, task and product retained its id. The guide reports a missing step, but task badges, product/process counts and client delivery silently omit the procedure; other Development/training/template references also lack delete-side reconciliation. | Preview dependencies and archive/tombstone, require reassignment, or transactionally detach under an explicit retention policy; fault-test every downstream surface, reload and partial failure. |
| Company capital/governance | **P1 — the UI-labelled authoritative register accepts contradictions.** A fresh round-trip retained duplicate class/owner ids, dangling class/owner/approval links, paid/allocation values above the declaration and 150% combined votes. Mounted owner/decision deletion strands movement and dividend links. | Validate the entire graph atomically with unique ids, real references and monetary/voting invariants; preserve or migrate linked history and browser-prove edit/delete, stale-save conflict and reload. |
| Battle Table history/concurrency | **P1 — stale whole-profile saves erase newer station changes and completed reviews are not actually locked.** Two snapshots both saved successfully; the later stale save reverted the first change. A completed review can return to draft and replace its decision/evidence without retaining a version. | Require focused or version-checked writes with visible conflict/merge; retain immutable completed evidence snapshots and create explicit amendments. Prove two-tab ordering, lock/amendment and reload in browser. |
| Legal-document retirement | **P1 — permanent Delete strands retained evidence links despite an existing archive state.** A fresh probe removed the legal row while a Finance obligation and approved governance decision retained its id. Finance silently loses the document action; governance still presents the raw id as linked. | Default to archive/tombstone, preview every dependant and require reassignment or an auditable detach/purge; coordinate row/binary state and fault-test every consumer plus reload. |
| Governance company scope | **P1 — selected-company snapshots mix agency-wide evidence and erasure targets.** An Alpha probe returned Beta's only DPA, both brands' clients and a Supabase agreement flag derived from Beta; only posture/HIPAA were actually scoped. | Label intentionally group-wide views or consistently filter company plus shared records, declarations, vendor matching and erasure targets; browser-prove every scope, creation, reload and failed-load state. |
...
| Radar sweep scheduling | **P2 — declared cadence and scope are not the executed schedule.** Evidence says hourly but runs only manual/daily; the daily tenant loop repeats app-wide Infra once per agency and an Infra failure prevents that tenant's evidence rollup. | Separate app-wide and tenant orchestration, schedule or relabel Evidence honestly, isolate failures and assert clock/call counts for zero/one/many agencies plus overlap/retry. |
| Application observability | **P1 — declared monitoring is unmounted scaffolding.** No production caller uses the Sentry/error or request-log wrappers, `@sentry/nextjs` is not installed, a DSN alone marks readiness ready, and the browser fallback still says the issue was logged. | Install/configure and mount real client/server/API capture, make readiness capability-based, prove a synthetic event reaches the sink and remove any unsupported logging claim. |
| Role-shell recovery navigation | **Partially repaired.** Agency-staff Account now returns to Team and Account/Permissions expose no owner/manager Settings link; isolated browser proof passes. Client/freelancer destinations and the agency-only portal 404 remain unresolved. | Centralise canonical home/settings links by current role/client scope and browser-prove profile, permissions and bad-deep-link recovery for every shell. |
| Customer install onboarding | **P2 — first-run setup promises a later Support path that is absent.** Password completion marks the whole welcome complete before installation; `/setup` then redirects away, while Support contains no install prompt or instructions. | Track install progress separately or expose a revisitable install/help surface, then prove prompt accept/decline and close/reopen on mobile and desktop. |
| Modal keyboard accessibility | **P1 — the declared modal contract is mostly visual.** Current source has 64 `aria-modal="true"` declarations across 50 TSX files, but only three of those files use the existing focus trap; 47 modal files do not contain/restore focus and only four of those handle Escape. | Standardise on one accessible dialog primitive or enforce the shared hook, accessible naming, deliberate initial focus, safe Escape and focus restoration; component-test and browser-tab representative nested/form/destructive dialogs. |
| Loading-state accessibility | **P2 — the Command Centre skeleton hides its own live status.** The route-loading root is `aria-hidden`, including its nested `role="status"`, so only sighted users receive progress feedback. | Hide decorative skeleton nodes separately, expose one useful live status and verify announcement, removal and focus continuity. |
| Responsive/accessibility verification | **P2 — the automated UX smoke is still HTTP markup smoke, but a representative manual browser matrix now exists.** On 2026-08-25 public, agency, client, customer, editor, Dev Team, staff and freelancer surfaces were genuinely rendered at 1280×720, 768×1024 and 375×812 (editor also 390×844). The eight-pixel Freelancer overflow found by that pass is fixed and browser-regressed. | Automate the coherent viewport matrix. Keyboard/focus, accessibility-tree/axe, screen-reader announcements, installability, forced errors, date boundaries and mutation journeys remain open, so #137 is only partially satisfied. |
| Composite-widget accessibility | **P2 — tab/menu/listbox roles over-promise keyboard behavior.** All 12 declared tablists and nine production menus omit roving/arrow-key navigation; Settings controls missing panel ids, and one editor listbox has no item-navigation model. Native child controls remain individually tabbable, but the announced composite contract is false. | Adopt tested shared primitives or remove the specialised roles; prove selected focus, arrow/Home/End, activation, Escape/return and actual panel relationships in browser. |
| Accessible control names | **P1 — central actions and visitor inputs can be announced without identity.** At least 13 manually confirmed internal icon-only actions have no accessible name, and published Contact, Booking, Newsletter, Product Search and custom Donation inputs rely on placeholder-only prompts. | Add visible/programmatic labels and repeated-row/state context, expose validation/status, enforce an AST/component guard and browser-inspect representative accessibility trees. |
...
| Root error recovery | **P2 — Aqua's custom `app/error.tsx` is not the true global boundary.** There is no `app/global-error.tsx`, so installed Next 16 uses its generic built-in fallback for root-layout/App Router failures, bypassing the branded recovery/capture path. | Add the required global client boundary with `html`/`body`, mount the proven capture path and production-browser fault both root and child segments through recovery. |
| Customer PWA installability | **P2 — the live manifest cannot trigger Chromium's custom install prompt.** It serves 192/180/32px icons only; Chromium requires both 192px and 512px before `beforeinstallprompt`, which is the event that reveals setup's Install button. | Add a genuine safe-zone-tested 512px icon, validate the served manifest and browser-prove eligible, accepted, dismissed, already-installed and ineligible states; keep #134's revisit lifecycle separate. |
| Published-block hydration | **P2 — default Share Buttons and auto Breadcrumb have different server/client first renders.** Static output encodes empty social targets and omits Breadcrumb; the client derives URL/path from `window`, while React 19 warns such hydration attributes are not patched. | Supply request URL/path in render context or use a hydration-stable deferred shape; server→hydrate-test default/explicit modes, navigation, functional links and zero recoverable errors. |
| Private media playback | **P2 — mounted audio and large training-media routes do not support HTTP byte ranges.** Inbox voice notes, call recordings and SOP media ignore `Range`, return full `200` bodies and sometimes fully buffer remote streams, despite accepted sizes up to 20/100/250 MB. | Centralise provider-aware `206`/`416` delivery and browser-prove metadata preload, prompt playback and seeking without transferring the complete object. |
| Voice/call capture | **P1 — all mounted recorders force WebM after testing only one WebM codec.** Unsupported construction is mislabeled as denied permission; the call flow persists the active call before recorder construction and can strand busy state plus live tracks when construction/start fails. | Centralise format negotiation and lifecycle cleanup/compensation; test WebM/MP4/default/unsupported branches, correct filenames/errors and every failure point across website, social, client and call capture. |
...
| Dev Team live-file performance | **Resolved for the bounded local/isolated-production and representative-browser phase.** Home no longer reaches the recursive roadmap/task/worker graph. Library scans only the 20 canonical documents, dynamically loads the selected view and does not prefetch sibling query views. Logs streams before the scanner/edit ledger, shares a compact exact-count snapshot and bounds the filesystem index. The non-destructive <2 GiB startup preflight remains. | Library measured **4.428→3.290s cold / 146→142ms warm**. Logs measured **3.182→0.857s first**, **2.702→0.868s post-TTL**, later warm **109ms TTFB / 252ms total**; eager graph **47/469,232B→3/15,433B**. Isolated production Library/Logs first HTTP maxed at **741.0ms**, repeat max at **29.0ms**, both 200/in budget. Desktop/mobile mounted acceptance is green. Deployed geo/CDN/provider latency remains operational acceptance. |
| Client-workspace 404 console | **Code/behaviour repaired; mounted console acceptance pending.** Root colour/sidebar bootstraps use identified Next 16.3 `beforeInteractive` components, preserve synchronous storage behavior and leave no raw root script; absent clients abort before chrome. | Dedicated **4/4**, focused **23/23**, wider client/navigation/editor-layout **125/125**, TypeScript pass. The isolated build was killed without a diagnostic and is inconclusive. Browser-prove valid/missing/generic-404 direct and client transitions with zero console errors and preserved state. |
| Page performance / hidden read writes | **The bounded speed phase is complete, not a deployed performance claim.** Agency retains station-only construction, intent-loaded Search, suppressed development prefetch, an idle session monitor and static manifest. Its latest retained dev baseline was **3.8s compile + 315ms app cold / 784ms warm**. The static proxy import closure then fell **1,139,995→255,050B (-77.6%)**, but a concurrent external `tsconfig` alias prevented a clean post-change runtime comparison. Isolated production Agency/Dev Team fresh-process first HTTP was **727.8/726.4ms** and repeat max **28.3/31.2ms**. | Desktop/mobile Day→Battle now carries one short-lived, revision- and element-access-bound result handle from a protected POST; GET navigation cannot replay a scan, and missing/provider-unavailable handles stay truthfully paused. Library/Logs and the responsive local matrix remain green. Measure deployed geo/CDN/provider latency separately; continue hidden-read correctness independently of speed. |
...
| Staff access | **Canonical Staff station projection and element-specific People DTOs are active; wider legacy policy convergence remains.** Navigation, Team pages, Staff People page/API projections and representative operations consume stable element levels; hidden elements do not receive the full people graph or dead Overview/Capacity links. A real Overview-only Staff identity saw only My Day and direct Pay/Actions attempts returned to the permitted Team surface; the clean follow-up loaded People Capacity at 390px without overflow/alert. | Finish any remaining proxy/page/API islands and retire or adapt the old HR station/freelancer policy sources. Complete positive Use/Manage and mutation/reload browser paths; the restricted-route proof alone is not the whole Staff matrix. |
| Finance event delivery | **Payment idempotency is durable; refund/dispute event deduplication is not.** The latter uses a process-local event-id set. | Persist event ids or make the refund/dispute transitions and their side effects durably idempotent; prove two-instance redelivery. |
| Documentation | **20 canonical Library volumes, with runtime compatibility preserved (2026-08-27).** | Nine authored subject volumes retain all **127 non-reference Markdown sources / 454,494 words** verbatim with original paths and SHA-256 provenance; eleven generated-reference documents remain. The founder-facing Dev Docs/Library index is exactly **20**. Compatibility fragments remain on disk only because Roadmap, Plans, Findings, Tasks and Updates still use them as live records. The complete documentation/parser/index/reference/file-finding gate passes **156/156**; TypeScript and focused diff checks are clean. |

These are tracked as issues **#16–#162** in [issues.md](issues.md). The earlier
...
|---|---|---|
| Dev Team → **the finish pass** (icons · accuracy · `?station=devteam`) | **Code-complete, behaviourally tested and browser-rendered.** The 2026-08-25 isolated pass opened the full workspace and recorded the old 9.2s home/4.7s Logs baseline. #151 now replaces repeated warm traversal with a coalesced bounded index; route-level browser re-timing remains before that performance residue closes. No plan, finding or doc was written during the earlier read-only pass. | `src/app/portal/dev-team/layout.tsx` · `src/lib/server/{devTeamBoard,devTeamAuditor,devTeamWorkers,devMarkdownCache}.ts` · `src/app/portal/dev-team/` · `scripts/smoke-dev-team-{portal,perf}.test.ts` · [issues #151](issues.md) |
| Dev Team → **API & MCP** (`/portal/dev-team/api`) | **Historical browser proof exists, but the management route is currently P0-unsafe.** The 2026-08-19 browser walk exercised create → reveal once → rotate → revoke and remains useful UI evidence. It does **not** prove current authorization safety: on 2026-08-24 an old owner cookie created a working external-AI token after the user was downgraded to staff because `/api/portal/settings/external-ai` trusts `getSessionFromRequest()` and the cookie role. Treat key management as not launch-safe until issue #22 is closed and regression-tested. | `src/app/api/portal/settings/external-ai/route.ts` · `src/lib/server/auth/auth.ts` · [issues #22](issues.md) |
