...
>
> Consolidated 2026-09-02 from **7** source documents / **86,184 words**. Each source is retained verbatim between provenance markers. The original path remains alongside it because relative links and runtime-backed Dev Team records still resolve from that location during the compatibility phase.
...
- [`docs/development/status.md`](#source-docs-development-status-md) — 20,750 words · `1ed419f21860`
...
<!-- AQUACRM_SOURCE_START path="docs/development/status.md" sha256="1ed419f2186092e848406338d8a354bdaf4bd8bd9581ae81536058a4ef87e04f" -->
...
> executed **6,417 tests across 1,093 suites: 6,415 passed / 0 failed / 2 skipped
> in 94,027.354917ms**; its subsequent Website Editor runner passed **49/49 files
> in 11.8s**. Chromium **151.0.7922.34** accounted for all **1,326** broad
...
> compiled in **47s**, completed TypeScript in **5.1s** and generated **245/245**
> static pages in **489ms**.
...
- The final canonical `npm run smoke:all` Node phase executed **6,417 tests across
  1,093 suites: 6,415 passed / 0 failed / 2 skipped in 94,027.354917ms**; its
  subsequent Website Editor runner passed **49/49 files in 11.8s**. This closes the
...
| Membership subscription and plan-price lifecycle | **Code/behaviour resolved; mounted/live-provider acceptance pending.** One per-user command converges paid→paid, paid→free, free→paid and cancellation. Plan create/price update uses the same durable pattern with exact base/candidate snapshots, separate monthly/annual checkpoints and stable provider identities outside the state transaction; stale targets cannot overwrite an intervening edit. | Subscription failure/retry/reload/concurrency proof **2/2**; dedicated plan-price proof **11/11**; earlier widened Membership/customer/discount **49/49** and package+lifecycle **11/11** remain green. The real foundation is wired; browser/live Stripe proof is not claimed. |
| Membership webhook delivery | **Code/behaviour resolved; signed live-provider acceptance pending.** A per-event scoped inbox retries failed/interrupted/legacy work and completes only after subscriber/payment state and synchronous side effects. Metadata/scope is required, payment rows are durable, event scope is real and processing failures return 503. | Fault/retry/fresh-container/concurrency/scope/payment proof **4/4**; combined Membership lifecycle+webhook **6/6**, widened **53/53**, package+dedicated **15/15**, TypeScript/diff pass. The remaining boundary is a signed live-provider delivery, not a missing foundation adapter. |
...
| Paid Memberships | **Code/behaviour repaired; live Stripe acceptance pending.** The foundation resolves the exact client Ecommerce install, returns unavailable without scoped credentials and supplies a real Stripe adapter otherwise. Subscription and plan-price operations persist intent/results around provider I/O with stable identities, exact target/reference revalidation and recoverable partial success. Partial default seeding is reported rather than hidden. | Lifecycle **2/2**, plan-price **11/11** and the earlier foundation/lifecycle/webhook module proof are green. Run paid plan → Checkout → signed webhook → switch/pause/resume/cancel → reload/reconciliation with a real Stripe test account. |
...
| Manifest plugin settings | **P2 partly repaired.** Shared settings are reachable across the registered families; Marketing, Website Editor and Fulfillment now consume truthful retained fields, and unwired controls identify themselves at the input. The source-derived inventory is **12 manifests / 43 fields: 24 consumed, 19 unwired**. | Exact settings/lifecycle proof is **26/26**. Wire or remove the remaining HR (3), Memberships (4), Leads Pipeline (3), Public Funnel (2), Ecommerce (2), Affiliates (2), Client CRM (2) and Finance (1) fields, then mounted-prove save/reload/changed behavior. |
