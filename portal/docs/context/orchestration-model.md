...
5. **The dev server / `.next`** — workers doing runtime verification need their own port (`npm run dev:verify`); don't assume a shared server (see the server blocker in [state.md](state.md)).
...
7. **Never commit/push/deploy** without Ed — every worker, every time.
8. **The auditor is read-only on source.** It never edits source or tests — it writes only [audits.md](../development/audits.md) — so it can verify a worker's files while that worker is still live, with zero collision risk. It uses its own `dev:verify` port. It reports findings; the **builder** does any rework (never the auditor — that would break rule 1).
