...
| `/api/portal/governance/subject-access` | POST / PUT / PATCH | GDPR Art. 15/20 staged safe JSON export (owner/manager). Every body is bounded and binds an exact same-agency open identity-verified access/portability request. POST walks every hydrated collection (including the explicitly loaded lazy sidecar), applies typed owner/scope vetoes and collection-specific projections, then durably stages replayable bounded bytes/digest without fulfilment. PUT records required human-review evidence against that digest. PATCH alone records separate delivery evidence and fulfils; all transitions are atomic with activity and every response is no-store. Scope excludes client-owned/provider stores. | agency | new 2026-08-28; hardened 2026-09-12 |
...
| **Subject access & portability (GDPR Art. 15/20)** | `lib/server/compliance/subjectAccessExport.ts` walks EVERY hydrated PortalState collection (explicitly loading lazy sidecars), while exact typed Person/client/facet/relationship ownership and nested owner/scope conflicts decide authority. Free-text and actor/assignee mentions never authorise. Collection-specific projections preserve safe Person history, finance ledger data and exact plugin install/client lineage; unknown PII, ambiguous/co-mingled/free-text/deep/unsupported shapes become value-free review counts. Door: `app/api/portal/governance/subject-access/` (owner/manager): bounded POST stages replayable bytes/digest and stays open; PUT records review evidence; PATCH records delivery evidence and only then fulfils, atomically with activity. All responses are no-store; caps fail explicitly; external stores remain out of scope. Tests: `scripts/smoke-subject-access-export.test.ts`, `scripts/smoke-subject-access-export-security.test.ts`, `scripts/smoke-subject-requests.test.ts`. New 2026-08-28; hardened 2026-09-12 |
