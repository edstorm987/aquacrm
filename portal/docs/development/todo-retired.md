...
- [x] **Public bucket wiring — RETIRED HISTORICAL MILESTONE (2026-08-19), superseded for security on 2026-09-10.** The old functional `upsert/getPublicUrl` and fail-open-walker completion claim is not the current release state. The corrective branch makes block-tree data-URL publication fail closed, removes the dormant app-server Supabase upload branch and makes the compatibility delete helper ownership-blocked. Remote public publication is a red production gate pending the containment migration, atomic page/object lifecycle, reference-safe recovery/recall and scanner/provider proof. See [current status](status.md) and the [current plan](plans/public-bucket.md).
