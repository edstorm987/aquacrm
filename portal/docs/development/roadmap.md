...
**Files:** src/archive/multi-agency/components/AgencySwitcher.tsx, src/archive/multi-agency/components/TenantSwitcher.tsx, src/archive/multi-agency/api, src/lib/authBrand.ts, src/app/login/page.tsx, src/app/api/auth/login/route.ts, src/server/tradingCompanies.ts
...
**Why:** A trading company is the cheap start; when it becomes a real business it gets its own agency, and the company switcher moves between them.

ED'S DECISION ON HISTORY (2026-08-20): it is a SELECTION at promotion time, not a fixed rule. Either MOVE ALL RECORDS, or START BLANK from a seed and IMPORT afterwards — and the import is PER RECORD TYPE, ticked or not. Ed called out enquiries specifically: 'if we still want to import enquiries here or not, we can.' WHY THIS SHAPE IS RIGHT: it decouples creating the tenant from moving the data. Creating the agency becomes cheap, safe and reversible; relocating live records across a tenant boundary — under an audit trail that assumed they never move — becomes an explicit, repeatable, verifiable import done in slices, instead of one irreversible migration. A promotion can happen the moment the business is real, with data following later. ⚠ ENQUIRIES ARE THE ONE WITH A LEGAL EDGE, and it is why making them optional is right rather than merely convenient. brand_enquiries carries consent / consentPurpose / consentVersion / consentCapturedAt — consent given to ONE legal entity does not automatically travel to a NEW one. So the importer must move the consent record WITH the enquiry, never the enquiry alone, and the UI must not present importing them as a neutral tick-box. There are 35 real enquiries live today, and docs/compliance/erasure-dpo-pack.md exists — this belongs in the DPO conversation, not in a developer's judgement call. REUSES WORK ALREADY DESIGNED: 'start blank + seed' IS seed zero plus the company's own brand kit, which TradingCompany already carries. EXISTS: eight record types carry company ownership (Client.companyId, ServerUser.companyIds, AgencyProduct, ExperiencePackage, ClientDelightRecord, CompanyProfile, LegalDocument, DevelopmentResource) plus routedCompanyId and WebsiteSourceDestination{kind:'company'}; bootstrapAgency creates agencies; src/archive/multi-agency/api/agency-add.ts is the archived create-tenant skeleton. ⚠ The importer must DERIVE its collection list from PortalState — the tenant wipe hand-lists and clears only 25 of 78, and an import
...
**Files:** docs/development/plans/advisor-omega-upgrade.md, docs/workspace/advisor.md, scripts/smoke-advisor-actions.test.ts, scripts/smoke-advisor-skills.test.ts, src/app/api/assistant/route.ts, src/app/api/portal/advisor/skills/route.ts, src/app/portal/agency/assistant/AssistantWorkspace.tsx, src/components/chrome/GlobalAdvisorDrawer.tsx, src/lib/advisorActions.ts, src/lib/advisorSkills.ts, src/lib/server/advisorSkillContext.ts, src/lib/server/advisorSkills.ts, src/lib/server/openaiAssistant.ts, src/server/customAIs.ts
