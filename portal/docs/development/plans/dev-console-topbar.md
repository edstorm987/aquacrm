...
- **Badge:** live count of things needing attention — open findings + open launch blockers (reuse
  `listFindings()` in `src/lib/server/devTeamFindings.ts` and `scanBlockers()` in
...
1. **Status at a glance** — open findings · open blockers · workers active now (reuse
   `scanWorkerSignals()` in `src/lib/server/devTeamWorkers.ts`, which already returns check-ins +
   file activity).
