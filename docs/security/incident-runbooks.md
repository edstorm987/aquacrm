...
1. **Suspend the account** (bites on the NEXT request, everywhere):
   `suspendUser(userId, "<you>", "<why>")`
2. **Revoke every session for the user**: `revokeAllUserSessions(userId, "<you>", "<why>")`
   — or one device only: `listUserSessions(userId)` → `revokeSession(sid, ...)`.
...
4. If the account had owner scope, **bump the tenant epoch** so anything minted
   while compromised dies: `bumpTenantSecurityEpoch(agencyId, ...)`.
5. Investigate with the audit trail + security events, then
   `unsuspendUser(userId, "<you>")` once re-credentialed.
...
1. **Freeze writes, keep serving reads**: `setGlobalReadOnly("<you>", "<why>")`
...
4. **Thaw**: `clearGlobalReadOnly("<you>")`.
...
1. **Kill every session everywhere**: `bumpGlobalSecurityEpoch("<you>", "<why>")`
...
1. **Kill AI**: `disableAi("<you>", "<why>")` — the single adapter every
...
4. **Restore**: `enableAi("<you>")`. Quotas (`PORTAL_AI_CALLS_PER_HOUR`)
...
1. `setGlobalReadOnly(...)` first — never restore under live writes.
...
4. Repoint the app; `clearGlobalReadOnly(...)`; monitor the event spine.
