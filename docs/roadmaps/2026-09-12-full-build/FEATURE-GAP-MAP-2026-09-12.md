...
| Sticky Settings rail and independent settings-pane scroll | Missing | `SettingsTabs.tsx`; Agency layout | `SETTINGS-SCROLL-001` |
...
| Inactivity check-in prompt | Implemented conditionally, user-visible behavior unverified | `SmartWorkSessionMonitor.tsx`; portal layout | `CHECKIN-UX-001` |
| New login background/layout, CAPTCHA and Policies | Security/CAPTCHA exists; visual redesign active; destination is Privacy only because canonical Terms is not always served | Login page/form; `BotChallenge.tsx` | `LOGIN-UX-001`; policy route truth before release |
...
1. Finish active `SEC-006`, `PLUGIN-LINEAGE-001`, and `LOGIN-UX-001` with fresh
...
3. Connect the accepted Sales flow to canonical searchable SOP/Guide references,
   then run the disjoint Sales-staff, Settings-scroll and Check-in gates.
