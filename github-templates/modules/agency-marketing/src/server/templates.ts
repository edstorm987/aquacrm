...
export class TemplateService {
  ...
  async update(id: string, untrustedPatch: UpdateTemplatePatch, actor: UserId): Promise<EmailTemplate | null> {
    ...
    if (patch.name !== undefined && (typeof patch.name !== "string" || !patch.name.trim())) throw new Error("Template name required.");
    if (patch.subject !== undefined && (typeof patch.subject !== "string" || !patch.subject.trim())) throw new Error("Template subject required.");
    if (patch.bodyHtml !== undefined && (typeof patch.bodyHtml !== "string" || !patch.bodyHtml.trim())) throw new Error("Template body required.");
    if (patch.bodyText !== undefined && typeof patch.bodyText !== "string") throw new Error("Template text body must be text.");
    ...
    if (patch.status !== undefined && patch.status !== "active" && patch.status !== "archived") throw new Error("Template status is not supported.");
